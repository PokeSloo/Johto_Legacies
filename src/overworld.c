...
static bool8 CanLearnFlashInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        if (!GetMonData(&gPlayerParty[i], MON_DATA_SPECIES, NULL))
            break;
        if (!GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG) && CanLearnTeachableMove(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES2), MOVE_FLASH))
            return TRUE;
    }
    return FALSE;
}
...
static void TransitionMapMusic(void)
{
    if (FlagGet(FLAG_DONT_TRANSITION_MUSIC) != TRUE)
    {
        ...
        {
            if (currentMusic == MUS_UNDERWATER || currentMusic == MUS_HG_SURF)
                return;
