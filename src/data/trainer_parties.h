static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer1[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_LAIRON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout1[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout2[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    },
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout3[] = {
    {
    .iv = 0,
    .lvl = 8,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout4[] = {
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern1[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern2[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern3[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle1[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_MISDREAVUS,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_HAUNTER,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SANDYGAST,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntPetalburgWoods[] = {
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_HOUNDOUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marcel[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_STOUTLAND,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SIRFETCHD,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_DARMANITAN_GALARIAN,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alberto[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_BLUNDER_POLICY,
    .abilityNums = 2,
    .moves = {MOVE_HYPNOSIS, MOVE_MOONBLAST, MOVE_ROOST, MOVE_AIR_SLASH}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_PIDGEOTITE,
    .abilityNums = 2,
    .moves = {MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_ROOST, MOVE_UPROAR}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Ed[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_VENOMOTH,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_QUIVER_DANCE, MOVE_SLEEP_POWDER, MOVE_BUG_BUZZ, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_PINSIR,
    .heldItem = ITEM_PINSIRITE,
    .abilityNums = 0,
    .moves = {MOVE_X_SCISSOR, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_QUICK_ATTACK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern4[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Declan[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_GOLISOPOD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntRusturfTunnel[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst1[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_MEOWTH_ALOLAN,
    },
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_HOUNDOUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst2[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst3[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_SPINARAK,
    },
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_SIZZLIPEDE,
    },
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_STUNKY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMuseum1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMuseum2[] = {
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre2[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre3[] = {
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst4[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_SALANDIT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout5[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout6[] = {
    {
    .iv = 0,
    .lvl = 8,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fredrick[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_CRUSTLE,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_KLINKLANG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Matt[] = {
    {
    .iv = 50,
    .lvl = 9,
    .species = SPECIES_BELLSPROUT,
    },
    {
    .iv = 50,
    .lvl = 12,
    .species = SPECIES_HOOTHOOT,
    },
    {
    .iv = 50,
    .lvl = 10,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Zander[] = {
    {
    .iv = 0,
    .lvl = 8,
    .species = SPECIES_HOOTHOOT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ShellyWeatherInstitute[] = {
    {
    .iv = 50,
    .lvl = 17,
    .species = SPECIES_HOUNDOUR,
    },
    {
    .iv = 50,
    .lvl = 18,
    .species = SPECIES_SNEASEL_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ShellySeafloorCavern[] = {
    {
    .iv = 100,
    .lvl = 37,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 100,
    .lvl = 37,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Archie[] = {
    {
    .iv = 150,
    .lvl = 41,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 41,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leah[] = {
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_MISDREAVUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Daisy[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_JUMPLUFF,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_LILLIGANT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose1[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_PETILIL,
    },
    {
    .iv = 0,
    .lvl = 19,
    .species = SPECIES_FLOETTE,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_ELDEGOSS,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Felix[] = {
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_LUDICOLO,
    .moves = {MOVE_HYDRO_PUMP, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM, MOVE_FAKE_OUT}
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_KINGDRA,
    .moves = {MOVE_SURF, MOVE_ICE_BEAM, MOVE_DRAGON_PULSE, MOVE_FLASH_CANNON}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Violet[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_ELDEGOSS,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_APPLETUN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_LILLIGANT_HISUIAN,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_FLOETTE,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_ELDEGOSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_LILLIGANT_HISUIAN,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_FLORGES,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_ELDEGOSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_LILLIGANT_HISUIAN,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_FLORGES,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_ELDEGOSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rose5[] = {
    {
    .iv = 60,
    .lvl = 36,
    .species = SPECIES_LILLIGANT_HISUIAN,
    },
    {
    .iv = 60,
    .lvl = 36,
    .species = SPECIES_FLORGES,
    },
    {
    .iv = 60,
    .lvl = 36,
    .species = SPECIES_ELDEGOSS,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty1[] = {
    {
    .iv = 50,
    .lvl = 70,
    .species = SPECIES_PANGORO,
    .moves = {MOVE_HAMMER_ARM, MOVE_TAUNT, MOVE_BULLET_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = 50,
    .lvl = 71,
    .species = SPECIES_CRADILY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_TOXIC, MOVE_AMNESIA, MOVE_ANCIENT_POWER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Chip[] = {
    {
    .iv = 150,
    .lvl = 61,
    .species = SPECIES_SANDACONDA,
    .moves = {MOVE_SANDSTORM, MOVE_EARTHQUAKE, MOVE_SLAM, MOVE_COIL}
    },
    {
    .iv = 150,
    .lvl = 62,
    .species = SPECIES_DUGTRIO,
    .moves = {MOVE_EARTHQUAKE, MOVE_SUCKER_PUNCH, MOVE_STONE_EDGE, MOVE_AERIAL_ACE}
    },
    {
    .iv = 150,
    .lvl = 62,
    .species = SPECIES_GLISCOR,
    .moves = {MOVE_ACROBATICS, MOVE_X_SCISSOR, MOVE_SKY_UPPERCUT, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Foster[] = {
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_ONIX,
    .moves = {MOVE_STEALTH_ROCK, MOVE_ROCK_POLISH, MOVE_GYRO_BALL, MOVE_SMACK_DOWN}
    },
    {
    .iv = 100,
    .lvl = 25,
    .species = SPECIES_GRAVELER_ALOLAN,
    .moves = {MOVE_ROCK_THROW, MOVE_SMACK_DOWN, MOVE_THUNDER_PUNCH, MOVE_SELF_DESTRUCT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty2[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_PANGORO,
    .moves = {MOVE_HAMMER_ARM, MOVE_TAUNT, MOVE_BULLET_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_CRADILY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_TOXIC, MOVE_AMNESIA, MOVE_ANCIENT_POWER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty3[] = {
    {
    .iv = 150,
    .lvl = 74,
    .species = SPECIES_PANGORO,
    .moves = {MOVE_HAMMER_ARM, MOVE_TAUNT, MOVE_BULLET_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = 150,
    .lvl = 75,
    .species = SPECIES_CRADILY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_TOXIC, MOVE_AMNESIA, MOVE_ANCIENT_POWER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty4[] = {
    {
    .iv = 200,
    .lvl = 76,
    .species = SPECIES_PANGORO,
    .moves = {MOVE_HAMMER_ARM, MOVE_TAUNT, MOVE_BULLET_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = 200,
    .lvl = 77,
    .species = SPECIES_CRADILY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_TOXIC, MOVE_AMNESIA, MOVE_ANCIENT_POWER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Dusty5[] = {
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_PANGORO,
    .moves = {MOVE_HAMMER_ARM, MOVE_TAUNT, MOVE_BULLET_PUNCH, MOVE_CRUNCH}
    },
    {
    .iv = 250,
    .lvl = 79,
    .species = SPECIES_CRADILY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_TOXIC, MOVE_AMNESIA, MOVE_ANCIENT_POWER}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy1[] = {
    {
    .iv = 50,
    .lvl = 17,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 50,
    .lvl = 17,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy2[] = {
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 100,
    .lvl = 27,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy3[] = {
    {
    .iv = 150,
    .lvl = 30,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 150,
    .lvl = 30,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy4[] = {
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GabbyAndTy5[] = {
    {
    .iv = 250,
    .lvl = 36,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 250,
    .lvl = 36,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_GabbyAndTy6[] = {
    {
    .iv = 250,
    .lvl = 39,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SONIC_BOOM, MOVE_THUNDER_WAVE, MOVE_METAL_SOUND, MOVE_THUNDERBOLT}
    },
    {
    .iv = 250,
    .lvl = 39,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_ASTONISH, MOVE_STOMP, MOVE_SUPERSONIC, MOVE_HYPER_VOICE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola1[] = {
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_PUMPKABOO,
    },
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Austina[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_SUNFLORA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gwen[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_SLURPUFF,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_SHIINOTIC,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_GRANBULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola2[] = {
    {
    .iv = 10,
    .lvl = 36,
    .species = SPECIES_PUMPKABOO,
    },
    {
    .iv = 10,
    .lvl = 36,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 20,
    .lvl = 39,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola4[] = {
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lola5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_STOUTLAND,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky1[] = {
    {
    .iv = 10,
    .lvl = 33,
    .species = SPECIES_POLIWHIRL,
    .moves = {MOVE_BODY_SLAM, MOVE_MUD_SHOT, MOVE_WATERFALL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Simon[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_AZURILL,
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Charlie[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_COALOSSAL,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_SALAZZLE,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_CENTISKORCH,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_WATERFALL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky3[] = {
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_WATERFALL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky4[] = {
    {
    .iv = 30,
    .lvl = 43,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_WATERFALL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ricky5[] = {
    {
    .iv = 40,
    .lvl = 46,
    .species = SPECIES_POLIWRATH,
    .moves = {MOVE_BODY_SLAM, MOVE_EARTHQUAKE, MOVE_WATERFALL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Randall[] = {
    {
    .iv = 155,
    .lvl = 35,
    .species = SPECIES_MORPEKO_HANGRY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_TORMENT, MOVE_BITE}
    },
    {
    .iv = 155,
    .lvl = 35,
    .species = SPECIES_GRAVELER_ALOLAN,
    .heldItem = ITEM_NONE,
    .abilityNums = 1,
    .moves = {MOVE_STEALTH_ROCK, MOVE_ROCK_BLAST, MOVE_BULLDOZE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 255,
    .lvl = 36,
    .species = SPECIES_GALVANTULA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_SUCKER_PUNCH, MOVE_BUG_BITE, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Parker[] = {
    {
    .iv = 255,
    .lvl = 26,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TEETER_DANCE, MOVE_DIZZY_PUNCH, MOVE_FOCUS_PUNCH, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_George[] = {
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_ZEBSTRIKA,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_STOMP, MOVE_SPARK, MOVE_FLAME_CHARGE, MOVE_PURSUIT}
    },
    {
    .iv = 100,
    .lvl = 35,
    .species = SPECIES_VIKAVOLT,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_BUG_BUZZ, MOVE_AIR_SLASH, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Berke[] = {
    {
    .iv = 255,
    .lvl = 26,
    .species = SPECIES_VIGOROTH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FOCUS_ENERGY, MOVE_SLASH, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Braxton[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_WATER_SPOUT, MOVE_HYDRO_PUMP, MOVE_HEAVY_SLAM, MOVE_BOUNCE}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_GYARADOSITE,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_CRUNCH, MOVE_EARTHQUAKE, MOVE_WATERFALL}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Vincent[] = {
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_GOLBAT,
    .moves = {MOVE_TOXIC, MOVE_MEAN_LOOK, MOVE_WING_ATTACK, MOVE_POISON_FANG}
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_SCOLIPEDE,
    .moves = {MOVE_TOXIC, MOVE_VENOSHOCK, MOVE_BUG_BITE, MOVE_ROLLOUT}
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_MAROWAK_ALOLAN,
    .moves = {MOVE_BONEMERANG, MOVE_SHADOW_BONE, MOVE_FLAME_WHEEL, MOVE_HEX}
    },
    {
    .iv = 100,
    .lvl = 32,
    .species = SPECIES_HYPNO,
    .moves = {MOVE_NASTY_PLOT, MOVE_PSYCHIC, MOVE_HYPNOSIS, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leroy[] = {
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_SEVIPER,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_DURANT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton1[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ABSOL,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SKUNTANK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edgar[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_KLEFKI,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_YANMEGA,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_WYRDEER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Albert[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_SNORLAX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Samuel[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_REUNICLUS,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_LEAVANNY,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Vito[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_MUSHARNA,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_ARMALDO,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Owen[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_RAPIDASH_GALARIAN,
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_PROBOPASS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wilton5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Warren[] = {
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Mary[] = {
    {
    .iv = 255,
    .lvl = 26,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FEINT_ATTACK, MOVE_SHOCK_WAVE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Alexia[] = {
    {
    .iv = 155,
    .lvl = 35,
    .species = SPECIES_PACHIRISU,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_NUZZLE, MOVE_THUNDERBOLT, MOVE_CHARM, MOVE_U_TURN}
    },
    {
    .iv = 155,
    .lvl = 35,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_DOUBLE_TEAM, MOVE_THUNDER_WAVE, MOVE_GRASS_KNOT}
    },
    {
    .iv = 155,
    .lvl = 36,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_MAGNET,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_CONFUSE_RAY, MOVE_SIGNAL_BEAM, MOVE_FLAIL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Jody[] = {
    {
    .iv = 255,
    .lvl = 26,
    .species = SPECIES_ZANGOOSE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SLASH, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Wendy[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_GOLISOPOD,
    .moves = {MOVE_FIRST_IMPRESSION, MOVE_LIQUIDATION, MOVE_X_SCISSOR, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_MIGHTYENA,
    .moves = {MOVE_PLAY_ROUGH, MOVE_SUCKER_PUNCH, MOVE_ROAR, MOVE_CRUNCH}
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_DUGTRIO_ALOLAN,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_SUCKER_PUNCH, MOVE_STONE_EDGE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Keira[] = {
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .iv = 100,
    .lvl = 28,
    .species = SPECIES_HEATMOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke1[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_PERSIAN_ALOLAN,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_DRAGALGE,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_HONCHKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jennifer[] = {
    {
    .iv = 200,
    .lvl = 60,
    .species = SPECIES_ALCREMIE,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_GRANBULL,
    },
    {
    .iv = 200,
    .lvl = 61,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hope[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_SWANNA,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_ESCAVALIER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shannon[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_OBSTAGOON,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_GIGALITH,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Michelle[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_AUDINO,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_GENGAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Caroline[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_SWOOBAT,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_URSALUNA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Julie[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_TYRANITAR,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_DONPHAN,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_EXCADRILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brooke5[] = {
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 140,
    .lvl = 34,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Patricia[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_DRIFLOON,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_TORKOAL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kindra[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_RUNERIGUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tammy[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_DHELMISE,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ORICORIO_SENSU,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_DRIFBLIM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie1[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tasha[] = {
    {
    .iv = 50,
    .lvl = 8,
    .species = SPECIES_BELLSPROUT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie2[] = {
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_DUSCLOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie3[] = {
    {
    .iv = 20,
    .lvl = 45,
    .species = SPECIES_DUSCLOPS,
    },
    {
    .iv = 20,
    .lvl = 45,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie4[] = {
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Valerie5[] = {
    {
    .iv = 40,
    .lvl = 52,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .iv = 40,
    .lvl = 52,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 40,
    .lvl = 52,
    .species = SPECIES_RUNERIGUS,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy1[] = {
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_MINCCINO,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemCustomMoves sParty_Daphne[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_NOIVERN,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_PULSE, MOVE_SUPER_FANG, MOVE_WHIRLWIND, MOVE_ROOST}
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_DRAMPA,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_PULSE, MOVE_EXTRASENSORY, MOVE_HYPER_VOICE, MOVE_GLARE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter2[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 0,
    .lvl = 28,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Cindy2[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_MINCCINO,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_DOUBLE_SLAP, MOVE_TICKLE, MOVE_KNOCK_OFF, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brianna[] = {
    {
    .iv = 150,
    .lvl = 56,
    .species = SPECIES_DRAGAPULT,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DARTS, MOVE_DRAGON_DANCE, MOVE_U_TURN, MOVE_PHANTOM_FORCE}
    },
    {
    .iv = 150,
    .lvl = 57,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_FLY, MOVE_FLAMETHROWER, MOVE_DRAGON_CLAW, MOVE_ZEN_HEADBUTT}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Naomi[] = {
    {
    .iv = 100,
    .lvl = 45,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy3[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_CINCCINO,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy4[] = {
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_CINCCINO,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Cindy5[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_CINCCINO,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemCustomMoves sParty_Cindy6[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_ROSERADE,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_GIGA_DRAIN, MOVE_GRASS_WHISTLE, MOVE_PROTECT, MOVE_TOXIC}
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_CINCCINO,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_SLAM, MOVE_WAKE_UP_SLAP, MOVE_KNOCK_OFF, MOVE_SING}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Melissa[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_TALONFLAME,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_ARBOK,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_LIEPARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sheila[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_WEEZING,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_MUK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shirley[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_NOCTOWL,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_LYCANROC,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_LUXRAY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica1[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_AQUA_TAIL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Connie[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_DURALUDON,
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_HAXORUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bridget[] = {
    {
    .iv = 150,
    .lvl = 56,
    .species = SPECIES_APPLETUN,
    },
    {
    .iv = 150,
    .lvl = 57,
    .species = SPECIES_KOMMO_O,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Olivia[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_GABITE,
    .moves = {MOVE_DRAGON_RUSH, MOVE_DIG, MOVE_STONE_EDGE, MOVE_AERIAL_ACE}
    },
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_DRAGON_PULSE, MOVE_MOONBLAST, MOVE_COTTON_GUARD, MOVE_ROOST}
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_GARCHOMP,
    .moves = {MOVE_DRAGON_CLAW, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_FIRE_FANG}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tiffany[] = {
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_TURTONATOR,
    },
    {
    .iv = 100,
    .lvl = 57,
    .species = SPECIES_GOODRA,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica2[] = {
    {
    .iv = 110,
    .lvl = 72,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 110,
    .lvl = 73,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_AQUA_TAIL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica3[] = {
    {
    .iv = 150,
    .lvl = 74,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 150,
    .lvl = 75,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_AQUA_TAIL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica4[] = {
    {
    .iv = 200,
    .lvl = 76,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 200,
    .lvl = 77,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_AQUA_TAIL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jessica5[] = {
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_PLAY_ROUGH, MOVE_IRON_HEAD, MOVE_CRUNCH, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 250,
    .lvl = 79,
    .species = SPECIES_FLOATZEL,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_AQUA_TAIL, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston1[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_NIDORAN_M,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mollie[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Garret[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston2[] = {
    {
    .iv = 0,
    .lvl = 27,
    .species = SPECIES_NIDORINO,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston3[] = {
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Winston4[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winston5[] = {
    {
    .iv = 0,
    .lvl = 35,
    .species = SPECIES_BRELOOM,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_COUNTER, MOVE_MACH_PUNCH, MOVE_SEED_BOMB, MOVE_DRAIN_PUNCH}
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_NUGGET,
    .abilityNums = 0,
    .moves = {MOVE_POISON_JAB, MOVE_SUCKER_PUNCH, MOVE_CHIP_AWAY, MOVE_IRON_TAIL}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve1[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_POLTEAGEIST,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_DUSCLOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia1[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_TALONFLAME,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_ARBOK,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_LIEPARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mark[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_CRABRAWLER,
    },
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_HELIOPTILE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtChimney1[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ABSOL,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SKUNTANK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve2[] = {
    {
    .iv = 10,
    .lvl = 56,
    .species = SPECIES_POLTEAGEIST,
    },
    {
    .iv = 10,
    .lvl = 57,
    .species = SPECIES_DUSCLOPS,
    },
    {
    .iv = 10,
    .lvl = 57,
    .species = SPECIES_TYPHLOSION_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve3[] = {
    {
    .iv = 20,
    .lvl = 58,
    .species = SPECIES_POLTEAGEIST,
    },
    {
    .iv = 20,
    .lvl = 59,
    .species = SPECIES_DUSCLOPS,
    },
    {
    .iv = 20,
    .lvl = 59,
    .species = SPECIES_TYPHLOSION_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve4[] = {
    {
    .iv = 30,
    .lvl = 60,
    .species = SPECIES_POLTEAGEIST,
    },
    {
    .iv = 30,
    .lvl = 61,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .iv = 30,
    .lvl = 61,
    .species = SPECIES_TYPHLOSION_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Steve5[] = {
    {
    .iv = 40,
    .lvl = 62,
    .species = SPECIES_POLTEAGEIST,
    },
    {
    .iv = 40,
    .lvl = 63,
    .species = SPECIES_DUSKNOIR,
    },
    {
    .iv = 40,
    .lvl = 63,
    .species = SPECIES_TYPHLOSION_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Luis[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_TIRTOUGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dominik[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_PORYGON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Douglas[] = {
    {
    .iv = 10,
    .lvl = 4,
    .species = SPECIES_KRICKETOT,
    },
    {
    .iv = 10,
    .lvl = 5,
    .species = SPECIES_BLIPBUG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darrin[] = {
    {
    .iv = 10,
    .lvl = 23,
    .species = SPECIES_DIGGERSBY,
    },
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_MORPEKO,
    },
    {
    .iv = 10,
    .lvl = 24,
    .species = SPECIES_GRANBULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony1[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerome[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_MINIOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Matthew[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_MURKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_David[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_HATTREM,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Spencer[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_BRUXISH,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_GASTRODON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Roland[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Nolen[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_JELLICENT,
    .moves = {MOVE_RECOVER, MOVE_WATER_PULSE, MOVE_OMINOUS_WIND, MOVE_BRINE}
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_TENTACRUEL,
    .moves = {MOVE_POISON_JAB, MOVE_BRINE, MOVE_ACID_SPRAY, MOVE_BUBBLE_BEAM}
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_KABUTOPS,
    .moves = {MOVE_MEGA_DRAIN, MOVE_AQUA_JET, MOVE_ENDURE, MOVE_MUD_SHOT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Stan[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_RELICANTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Barry[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dean[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_PYUKUMUKU,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_WHISCASH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rodney[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Richard[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Herman[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_KINGDRA,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_CLAWITZER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Santiago[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gilbert[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_PYUKUMUKU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Franklin[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_ARAQUANID,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_SEISMITOAD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kevin[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_BRUXISH,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_SEISMITOAD,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_QUAGSIRE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jack[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_HUNTAIL,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_WATER_PULSE, MOVE_SCARY_FACE, MOVE_ICE_FANG}
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_OMASTAR,
    .moves = {MOVE_ANCIENT_POWER, MOVE_PROTECT, MOVE_BRINE, MOVE_MUD_SHOT}
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_GYARADOS,
    .moves = {MOVE_DRAGON_RAGE, MOVE_AQUA_TAIL, MOVE_ICE_FANG, MOVE_SCARY_FACE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dudley[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chad[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_SEISMITOAD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_KLEAVOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tony5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_KLEAVOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Takao[] = {
    {
    .iv = 127,
    .lvl = 16,
    .species = SPECIES_KLEFKI,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hitoshi[] = {
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_GURDURR,
    },
    {
    .iv = 200,
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kiyo[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_PASSIMIAN,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koichi[] = {
    {
    .iv = 100,
    .lvl = 21,
    .species = SPECIES_SWADLOON,
    },
    {
    .iv = 150,
    .lvl = 21,
    .species = SPECIES_BEEDRILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob1[] = {
    {
    .iv = 100,
    .lvl = 21,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob2[] = {
    {
    .iv = 110,
    .lvl = 27,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nob4[] = {
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_CENTISKORCH,
    },
    {
    .iv = 130,
    .lvl = 31,
    .species = SPECIES_SCIZOR,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Nob5[] = {
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_NINJASK,
    .heldItem = ITEM_FOCUS_SASH
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_KRICKETUNE,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_CENTISKORCH,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .species = SPECIES_SCIZOR,
    .heldItem = ITEM_EXPERT_BELT
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Yuji[] = {
    {
    .iv = 100,
    .lvl = 25,
    .species = SPECIES_TOXEL,
    },
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Daisuke[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_PRIMEAPE,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_HERACROSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Atsushi[] = {
    {
    .iv = 100,
    .lvl = 7,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Kirk[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_MEOWTH,
    .moves = {MOVE_FAKE_OUT, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_STUFFUL,
    .moves = {MOVE_FACADE, MOVE_PAYBACK, MOVE_FLAIL, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout7[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BELLSPROUT,
    },
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_HOOTHOOT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntAquaHideout8[] = {
    {
    .iv = 0,
    .lvl = 8,
    .species = SPECIES_HOOTHOOT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shawn[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_HERDIER,
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_FURFROU,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Fernando1[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_FLAME_ORB,
    .abilityNums = 2,
    .moves = {MOVE_FACADE, MOVE_WILD_CHARGE, MOVE_SUPERPOWER, MOVE_VOLT_SWITCH}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_HIDDEN_POWER, MOVE_UPROAR}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton1[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_VOLTORB_HISUIAN,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_HELIOPTILE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton2[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_VOLTORB_HISUIAN,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_HELIOLISK,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_VOLTORB_HISUIAN,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_HELIOLISK,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_ELECTRODE_HISUIAN,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_HELIOLISK,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dalton5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_ELECTRODE_HISUIAN,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_HELIOLISK,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_MAGNEZONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cole[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_BARRASKEWDA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeff[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 100,
    .lvl = 31,
    .species = SPECIES_GASTRODON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Axle[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_PYUKUMUKU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jace[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Keegan[] = {
    {
    .iv = 120,
    .lvl = 30,
    .species = SPECIES_ALOMOMOLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie1[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie2[] = {
    {
    .iv = 20,
    .lvl = 57,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 20,
    .lvl = 58,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie3[] = {
    {
    .iv = 40,
    .lvl = 59,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 40,
    .lvl = 60,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 40,
    .lvl = 60,
    .species = SPECIES_MANDIBUZZ,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie4[] = {
    {
    .iv = 60,
    .lvl = 61,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 60,
    .lvl = 62,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 60,
    .lvl = 62,
    .species = SPECIES_MANDIBUZZ,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bernie5[] = {
    {
    .iv = 80,
    .lvl = 63,
    .species = SPECIES_PIDGEOT,
    },
    {
    .iv = 80,
    .lvl = 64,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 80,
    .lvl = 64,
    .species = SPECIES_MANDIBUZZ,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Drew[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_WHIMSICOTT,
    .moves = {MOVE_MOONBLAST, MOVE_HURRICANE, MOVE_ENERGY_BALL, MOVE_TAILWIND}
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_LILLIGANT_HISUIAN,
    .moves = {MOVE_LEAF_BLADE, MOVE_CLOSE_COMBAT, MOVE_RECOVER, MOVE_SLEEP_POWDER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Beau[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_TROPIUS,
    .moves = {MOVE_LEAF_STORM, MOVE_BODY_SLAM, MOVE_WHIRLWIND, MOVE_AIR_SLASH}
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_BRELOOM,
    .moves = {MOVE_SEED_BOMB, MOVE_SKY_UPPERCUT, MOVE_MACH_PUNCH, MOVE_SPORE}
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_TREVENANT,
    .moves = {MOVE_DESTINY_BOND, MOVE_WOOD_HAMMER, MOVE_SHADOW_CLAW, MOVE_CONFUSE_RAY}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Larry[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_MEOWSTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shane[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_LUXRAY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Justin[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_KOFFING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan1[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_GLIGAR,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_NINETALES,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Autumn[] = {
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_JYNX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Travis[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_GRAPPLOCT,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_SAWK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan2[] = {
    {
    .iv = 20,
    .lvl = 51,
    .species = SPECIES_GLIGAR,
    },
    {
    .iv = 20,
    .lvl = 52,
    .species = SPECIES_NINETALES,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan3[] = {
    {
    .iv = 40,
    .lvl = 53,
    .species = SPECIES_GLISCOR,
    },
    {
    .iv = 40,
    .lvl = 54,
    .species = SPECIES_NINETALES,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan4[] = {
    {
    .iv = 60,
    .lvl = 55,
    .species = SPECIES_GLIGAR,
    },
    {
    .iv = 60,
    .lvl = 56,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 60,
    .lvl = 56,
    .species = SPECIES_DONPHAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ethan5[] = {
    {
    .iv = 80,
    .lvl = 57,
    .species = SPECIES_GLIGAR,
    },
    {
    .iv = 80,
    .lvl = 58,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 80,
    .lvl = 58,
    .species = SPECIES_DONPHAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brent[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_SLURPUFF,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_PINCURCHIN,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Donald[] = {
    {
    .iv = 100,
    .lvl = 19,
    .species = SPECIES_VULPIX,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_SALANDIT,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MURKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Taylor[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_DUSTOX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey1[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_SHEDINJA,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_KLEAVOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Derek[] = {
    {
    .iv = 150,
    .lvl = 36,
    .species = SPECIES_SCOLIPEDE,
    },
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_ORBEETLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey2[] = {
    {
    .iv = 150,
    .lvl = 63,
    .species = SPECIES_SHEDINJA,
    },
    {
    .iv = 150,
    .lvl = 64,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 150,
    .lvl = 64,
    .species = SPECIES_KLEAVOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey3[] = {
    {
    .iv = 200,
    .lvl = 66,
    .species = SPECIES_SHEDINJA,
    },
    {
    .iv = 200,
    .lvl = 67,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 200,
    .lvl = 67,
    .species = SPECIES_KLEAVOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jeffrey4[] = {
    {
    .iv = 250,
    .lvl = 69,
    .species = SPECIES_SHEDINJA,
    },
    {
    .iv = 250,
    .lvl = 70,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 250,
    .lvl = 70,
    .species = SPECIES_KLEAVOR,
    },
    {
    .iv = 250,
    .lvl = 70,
    .species = SPECIES_VOLCARONA,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Jeffrey5[] = {
    {
    .iv = 250,
    .lvl = 72,
    .species = SPECIES_SHEDINJA,
    },
    {
    .iv = 250,
    .lvl = 73,
    .species = SPECIES_FORRETRESS,
    },
    {
    .iv = 250,
    .lvl = 73,
    .species = SPECIES_KLEAVOR,
    },
    {
    .iv = 250,
    .lvl = 73,
    .species = SPECIES_VOLCARONA,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Edward[] = {
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_PYROAR,
    .moves = {MOVE_FLAMETHROWER, MOVE_CRUNCH, MOVE_HYPER_VOICE, MOVE_NOBLE_ROAR}
    },
    {
    .iv = 0,
    .lvl = 47,
    .species = SPECIES_COPPERAJAH,
    .moves = {MOVE_IRON_HEAD, MOVE_BULLDOZE, MOVE_HEAVY_SLAM, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Preston[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_GLALIE,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_AVALUGG,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_DEWGONG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Virgil[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_ABOMASNOW,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_CRYOGONAL,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_CRABOMINABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Blake[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_MR_MIME_GALARIAN,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_JYNX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_William[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_DARUMAKA,
    },
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_PANSAGE,
    },
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Joshua[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_STUNKY,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron1[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_REUNICLUS,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_XATU,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_WOBBUFFET,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron2[] = {
    {
    .iv = 10,
    .lvl = 33,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 33,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron3[] = {
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 38,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 41,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cameron5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Jaclyn[] = {
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_HAKAMO_O,
    .moves = {MOVE_SKY_UPPERCUT, MOVE_DRAGON_CLAW, MOVE_SCREECH, MOVE_DRAGON_TAIL}
    },
    {
    .iv = 0,
    .lvl = 47,
    .species = SPECIES_MIENSHAO,
    .moves = {MOVE_FAKE_OUT, MOVE_JUMP_KICK, MOVE_DRAIN_PUNCH, MOVE_MEDITATE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hannah[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_VANILLUXE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Samantha[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_CRYOGONAL,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_SEALEO,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_FROSLASS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maura[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_WEAVILE,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_EISCUE,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kayla[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_GASTLY,
    },
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_SINISTEA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alexis[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_INKAY,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_SNEASEL,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Jacki1[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_PSYCHIC, MOVE_EARTH_POWER, MOVE_TOXIC}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_GARDEVOIR,
    .heldItem = ITEM_GARDEVOIRITE,
    .abilityNums = 1,
    .moves = {MOVE_PSYSHOCK, MOVE_MOONBLAST, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki4[] = {
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 40,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacki5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_LUNATONE,
    },
    {
    .iv = 40,
    .lvl = 43,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Walter1[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_RELICANTH,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SEISMITOAD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Micah[] = {
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thomas[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_ZANGOOSE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Walter2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Walter5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_GOLDUCK,
    .moves = {MOVE_FURY_SWIPES, MOVE_DISABLE, MOVE_CONFUSION, MOVE_PSYCH_UP}
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Sidney[] = {
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_OVERQWIL,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 2,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_DESTINY_BOND, MOVE_CRUNCH, MOVE_GUNK_SHOT}
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_CROBAT,
    .heldItem = ITEM_SHARP_BEAK,
    .abilityNums = 2,
    .moves = {MOVE_BRAVE_BIRD, MOVE_SUPER_FANG, MOVE_U_TURN, MOVE_CROSS_POISON}
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_RAPID_SPIN, MOVE_SLUDGE_BOMB, MOVE_KNOCK_OFF}
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_SLOWKING_GALARIAN,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 0,
    .moves = {MOVE_PSYCHIC, MOVE_POWER_GEM, MOVE_NASTY_PLOT, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_LEECH_SEED, MOVE_SLUDGE_BOMB, MOVE_SYNTHESIS, MOVE_EARTH_POWER}
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_EARTH_POWER, MOVE_THUNDERBOLT, MOVE_SLUDGE_WAVE, MOVE_FIRE_BLAST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Phoebe[] = {
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_ROTOM_HEAT,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_SHADOW_BALL, MOVE_OVERHEAT, MOVE_VOLT_SWITCH, MOVE_PAIN_SPLIT}
    },
    {
    .iv = 255,
    .lvl = 67,
    .species = SPECIES_NINETALES,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 2,
    .moves = {MOVE_FIRE_BLAST, MOVE_SOLAR_BEAM, MOVE_NASTY_PLOT, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_DARMANITAN,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_FLARE_BLITZ, MOVE_EARTHQUAKE, MOVE_U_TURN, MOVE_HAMMER_ARM}
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_BLAZIKEN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_FLARE_BLITZ, MOVE_STONE_EDGE, MOVE_HIGH_JUMP_KICK, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_VOLCARONA,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_QUIVER_DANCE, MOVE_BUG_BUZZ, MOVE_FLAMETHROWER, MOVE_ROOST}
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_ARCANINE_HISUIAN,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 2,
    .moves = {MOVE_HEAD_SMASH, MOVE_FLARE_BLITZ, MOVE_EXTREME_SPEED, MOVE_WILD_CHARGE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Glacia[] = {
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_SPIRITOMB,
    .heldItem = ITEM_BLACK_GLASSES,
    .abilityNums = 2,
    .moves = {MOVE_SHADOW_BALL, MOVE_DARK_PULSE, MOVE_PSYCHIC, MOVE_WILL_O_WISP}
    },
    {
    .iv = 255,
    .lvl = 68,
    .species = SPECIES_MIMIKYU,
    .heldItem = ITEM_RED_CARD,
    .abilityNums = 0,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SHADOW_CLAW, MOVE_SHADOW_SNEAK, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_DRAGAPULT,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_DRAGON_DANCE, MOVE_DRAGON_DARTS, MOVE_PHANTOM_FORCE, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_DUSKNOIR,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_ICE_PUNCH, MOVE_SHADOW_SNEAK, MOVE_EARTHQUAKE, MOVE_PAIN_SPLIT}
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_AEGISLASH,
    .heldItem = ITEM_IRON_BALL,
    .abilityNums = 0,
    .moves = {MOVE_KINGS_SHIELD, MOVE_TOXIC, MOVE_GYRO_BALL, MOVE_SHADOW_BALL}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 0,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_WAVE, MOVE_THUNDERBOLT, MOVE_FOCUS_BLAST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Drake[] = {
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_ROCKY_HELMET,
    .abilityNums = 1,
    .moves = {MOVE_ROOST, MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_BRAVE_BIRD}
    },
    {
    .iv = 255,
    .lvl = 69,
    .species = SPECIES_DURALUDON,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_FLASH_CANNON, MOVE_DRAGON_PULSE, MOVE_THUNDERBOLT, MOVE_DARK_PULSE}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AQUA_JET, MOVE_DRILL_PECK, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_FERROTHORN,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_POWER_WHIP, MOVE_GYRO_BALL, MOVE_LEECH_SEED}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_EXCADRILL,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_METEOR_MASH, MOVE_EARTHQUAKE, MOVE_ZEN_HEADBUTT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne1[] = {
    {
    .iv = 100,
    .lvl = 12,
    .species = SPECIES_PIKIPEK,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_AERIAL_ACE, MOVE_ECHOED_VOICE, MOVE_ROCK_SMASH, MOVE_TAILWIND}
    },
    {
    .iv = 100,
    .lvl = 12,
    .species = SPECIES_HOOTHOOT,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_AERIAL_ACE, MOVE_CONFUSION, MOVE_HYPNOSIS, MOVE_WHIRLWIND}
    },
    {
    .iv = 180,
    .lvl = 14,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAND_ATTACK, MOVE_QUICK_ATTACK, MOVE_GUST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly1[] = {
    {
    .iv = 150,
    .lvl = 18,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_KINGS_ROCK,
    .abilityNums = 0,
    .moves = {MOVE_AQUA_JET, MOVE_TAIL_WHIP, MOVE_DRAINING_KISS, MOVE_ICE_PUNCH}
    },
    {
    .iv = 150,
    .lvl = 19,
    .species = SPECIES_MIMIKYU,
    .heldItem = ITEM_BRIGHT_POWDER,
    .abilityNums = 0,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_DRAINING_KISS, MOVE_BABY_DOLL_EYES, MOVE_ASTONISH}
    },
    {
    .iv = 170,
    .lvl = 19,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 1,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_THUNDER_PUNCH, MOVE_DRAINING_KISS, MOVE_KNOCK_OFF}
    },
    {
    .iv = 200,
    .lvl = 20,
    .species = SPECIES_SYLVEON,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_DRAINING_KISS, MOVE_BABY_DOLL_EYES, MOVE_QUICK_ATTACK, MOVE_STORED_POWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson1[] = {
    {
    .iv = 200,
    .lvl = 26,
    .species = SPECIES_WIGGLYTUFF,
    .heldItem = ITEM_BRIGHT_POWDER,
    .abilityNums = 0,
    .moves = {MOVE_DAZZLING_GLEAM, MOVE_HYPER_VOICE, MOVE_DISABLE, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = 200,
    .lvl = 26,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FLAME_ORB,
    .abilityNums = 0,
    .moves = {MOVE_FACADE, MOVE_U_TURN, MOVE_BRAVE_BIRD, MOVE_QUICK_ATTACK}
    },
    {
    .iv = 220,
    .lvl = 27,
    .species = SPECIES_LOPUNNY,
    .heldItem = ITEM_EXPERT_BELT,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_RETURN, MOVE_JUMP_KICK, MOVE_ICE_PUNCH}
    },
    {
    .iv = 220,
    .lvl = 27,
    .species = SPECIES_GIRAFARIG,
    .heldItem = ITEM_TWISTED_SPOON,
    .abilityNums = 0,
    .moves = {MOVE_PSYCHIC, MOVE_DOUBLE_HIT, MOVE_SHADOW_BALL, MOVE_AGILITY}
    },
    {
    .iv = 250,
    .lvl = 28,
    .species = SPECIES_MILTANK,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_BODY_SLAM, MOVE_MILK_DRINK, MOVE_ROLLOUT, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery1[] = {
    {
    .iv = 200,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_DAMP_ROCK,
    .abilityNums = 1,
    .moves = {MOVE_SCALD, MOVE_U_TURN, MOVE_ROOST, MOVE_HURRICANE}
    },
    {
    .iv = 200,
    .lvl = 34,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_GIGA_DRAIN, MOVE_ICE_BEAM, MOVE_RAIN_DANCE}
    },
    {
    .iv = 250,
    .lvl = 34,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_VOLT_SWITCH}
    },
    {
    .iv = 250,
    .lvl = 35,
    .species = SPECIES_TOXAPEX,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_HAZE, MOVE_RECOVER, MOVE_BANEFUL_BUNKER}
    },
    {
    .iv = 250,
    .lvl = 35,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 2,
    .moves = {MOVE_EARTHQUAKE, MOVE_SCALD, MOVE_REST, MOVE_BOUNCE}
    },
    {
    .iv = 250,
    .lvl = 36,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_SHELL_BELL,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_WATERFALL, MOVE_ICE_FANG}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman1[] = {
    {
    .iv = 250,
    .lvl = 40,
    .species = SPECIES_ELECTRODE_HISUIAN,
    .heldItem = ITEM_HEAVY_DUTY_BOOTS,
    .abilityNums = 1,
    .moves = {MOVE_VOLT_SWITCH, MOVE_ENERGY_BALL, MOVE_THUNDERBOLT, MOVE_EXPLOSION}
    },
    {
    .iv = 250,
    .lvl = 40,
    .species = SPECIES_GOLEM_ALOLAN,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 1,
    .moves = {MOVE_STONE_EDGE, MOVE_EXPLOSION, MOVE_EARTHQUAKE, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = 250,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_OVERHEAT, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 250,
    .lvl = 41,
    .species = SPECIES_EELEKTROSS,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_GIGA_DRAIN, MOVE_FLAMETHROWER, MOVE_KNOCK_OFF}
    },
    {
    .iv = 250,
    .lvl = 40,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_VOLT_SWITCH, MOVE_HYDRO_PUMP, MOVE_PAIN_SPLIT, MOVE_THUNDERBOLT}
    },
    {
    .iv = 250,
    .lvl = 42,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .abilityNums = 0,
    .moves = {MOVE_OVERDRIVE, MOVE_BOOMBURST, MOVE_SLUDGE_BOMB, MOVE_VOLT_SWITCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona1[] = {
    {
    .iv = 250,
    .lvl = 45,
    .species = SPECIES_PASSIMIAN,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 2,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_KNOCK_OFF, MOVE_IRON_HEAD}
    },
    {
    .iv = 250,
    .lvl = 45,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SUCKER_PUNCH, MOVE_DRAIN_PUNCH, MOVE_POISON_JAB}
    },
    {
    .iv = 250,
    .lvl = 45,
    .species = SPECIES_HERACROSS,
    .heldItem = ITEM_BLUNDER_POLICY,
    .abilityNums = 2,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_MEGAHORN, MOVE_STONE_EDGE, MOVE_BULLET_SEED}
    },
    {
    .iv = 250,
    .lvl = 46,
    .species = SPECIES_SCRAFTY,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_DRAGON_DANCE, MOVE_CRUNCH, MOVE_HIGH_JUMP_KICK, MOVE_STONE_EDGE}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 2,
    .moves = {MOVE_EARTHQUAKE, MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT}
    },
    {
    .iv = 255,
    .lvl = 47,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 1,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_DYNAMIC_PUNCH, MOVE_KNOCK_OFF, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza1[] = {
    {
    .iv = 250,
    .lvl = 50,
    .species = SPECIES_FROSLASS,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 2,
    .moves = {MOVE_DESTINY_BOND, MOVE_SHADOW_BALL, MOVE_BLIZZARD, MOVE_SPIKES}
    },
    {
    .iv = 250,
    .lvl = 50,
    .species = SPECIES_MR_RIME,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_PSYCHIC, MOVE_FREEZE_DRY, MOVE_BRICK_BREAK, MOVE_SLACK_OFF}
    },
    {
    .iv = 250,
    .lvl = 50,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_FREEZE_DRY, MOVE_HYDRO_PUMP, MOVE_BLIZZARD, MOVE_PSYCHIC}
    },
    {
    .iv = 250,
    .lvl = 51,
    .species = SPECIES_GLACEON,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 0,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_HIDDEN_POWER, MOVE_FREEZE_DRY}
    },
    {
    .iv = 250,
    .lvl = 51,
    .species = SPECIES_MAMOSWINE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_EARTHQUAKE, MOVE_ICE_SHARD, MOVE_ICICLE_CRASH, MOVE_KNOCK_OFF}
    },
    {
    .iv = 250,
    .lvl = 52,
    .species = SPECIES_DARMANITAN_GALARIAN,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_ICICLE_CRASH, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan1[] = {
    {
    .iv = 250,
    .lvl = 60,
    .species = SPECIES_DRAGALGE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 2,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_TOXIC_SPIKES, MOVE_HYDRO_PUMP, MOVE_DRAGON_PULSE}
    },
    {
    .iv = 250,
    .lvl = 60,
    .species = SPECIES_KOMMO_O,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 2,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_CLANGING_SCALES, MOVE_CLOSE_COMBAT, MOVE_POISON_JAB}
    },
    {
    .iv = 250,
    .lvl = 60,
    .species = SPECIES_GOODRA_HISUIAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 2,
    .moves = {MOVE_FLASH_CANNON, MOVE_FIRE_BLAST, MOVE_SCALD, MOVE_DRAGON_PULSE}
    },
    {
    .iv = 250,
    .lvl = 61,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 2,
    .moves = {MOVE_DRAGON_CLAW, MOVE_EARTHQUAKE, MOVE_POISON_JAB, MOVE_STONE_EDGE}
    },
    {
    .iv = 250,
    .lvl = 61,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 2,
    .moves = {MOVE_HYDRO_PUMP, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM, MOVE_FLASH_CANNON}
    },
    {
    .iv = 250,
    .lvl = 62,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry1[] = {
    {
    .iv = 10,
    .lvl = 13,
    .species = SPECIES_ROGGENROLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ted[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_GOLBAT,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_MAREANIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paul[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_MEOWTH_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_BOLDORE,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_BOLDORE,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_BOLDORE,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jerry5[] = {
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_BASTIODON,
    },
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_GIGALITH,
    },
    {
    .iv = 50,
    .lvl = 34,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen1[] = {
    {
    .iv = 10,
    .lvl = 12,
    .species = SPECIES_GLAMEOW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Georgia[] = {
    {
    .iv = 10,
    .lvl = 25,
    .species = SPECIES_NIDORINA,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_PONYTA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_GLAMEOW,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_CLEFAIRY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_GLAMEOW,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_CLEFABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_GLAMEOW,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_CLEFABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Karen5[] = {
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_PURUGLY,
    },
    {
    .iv = 50,
    .lvl = 35,
    .species = SPECIES_CLEFABLE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_KateAndJoy[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SEAKING,
    .moves = {MOVE_WATERFALL, MOVE_KNOCK_OFF, MOVE_DRILL_RUN, MOVE_MEGAHORN}
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_OMASTAR,
    .moves = {MOVE_SHELL_SMASH, MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_EARTH_POWER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg1[] = {
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_ELECTABUZZ,
    .moves = {MOVE_SHOCK_WAVE, MOVE_SWIFT, MOVE_LOW_KICK, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_MAGMAR,
    .moves = {MOVE_FIRE_SPIN, MOVE_FEINT_ATTACK, MOVE_SMOKESCREEN, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_ELECTABUZZ,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_THUNDERBOLT, MOVE_LOW_KICK, MOVE_THUNDER_WAVE}
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MAGMAR,
    .moves = {MOVE_FLAME_BURST, MOVE_FEINT_ATTACK, MOVE_CONFUSE_RAY, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_ELECTABUZZ,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_THUNDERBOLT, MOVE_LOW_KICK, MOVE_THUNDER_WAVE}
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_MAGMAR,
    .moves = {MOVE_FLAME_BURST, MOVE_FEINT_ATTACK, MOVE_CONFUSE_RAY, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_ELECTIVIRE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_THUNDERBOLT, MOVE_LOW_KICK, MOVE_THUNDER_WAVE}
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_MAGMORTAR,
    .moves = {MOVE_LAVA_PLUME, MOVE_FEINT_ATTACK, MOVE_CONFUSE_RAY, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AnnaAndMeg5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_ELECTIVIRE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_THUNDERBOLT, MOVE_LOW_KICK, MOVE_THUNDER_WAVE}
    },
    {
    .iv = 40,
    .lvl = 38,
    .species = SPECIES_MAGMORTAR,
    .moves = {MOVE_LAVA_PLUME, MOVE_FEINT_ATTACK, MOVE_CONFUSE_RAY, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Victor[] = {
    {
    .iv = 25,
    .lvl = 16,
    .species = SPECIES_TAILLOW,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 25,
    .lvl = 16,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel1[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemCustomMoves sParty_Colton[] = {
    {
    .iv = 0,
    .lvl = 22,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 40,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel2[] = {
    {
    .iv = 0,
    .lvl = 39,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel3[] = {
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel4[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Miguel5[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Victoria[] = {
    {
    .iv = 50,
    .lvl = 17,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Vanessa[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_SIMIPOUR,
    .heldItem = ITEM_LIFE_ORB
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_LEFTOVERS
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Bethany[] = {
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_DRUDDIGON,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 100,
    .lvl = 57,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel1[] = {
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 0,
    .lvl = 47,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel2[] = {
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel3[] = {
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel4[] = {
    {
    .iv = 60,
    .lvl = 58,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 60,
    .lvl = 58,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Isabel5[] = {
    {
    .iv = 80,
    .lvl = 60,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 80,
    .lvl = 60,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Timothy1[] = {
    {
    .iv = 200,
    .lvl = 21,
    .species = SPECIES_MUNCHLAX,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy2[] = {
    {
    .iv = 210,
    .lvl = 33,
    .species = SPECIES_SNORLAX,
    .moves = {MOVE_BODY_SLAM, MOVE_REST, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy3[] = {
    {
    .iv = 220,
    .lvl = 36,
    .species = SPECIES_SNORLAX,
    .moves = {MOVE_BODY_SLAM, MOVE_REST, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy4[] = {
    {
    .iv = 230,
    .lvl = 39,
    .species = SPECIES_SNORLAX,
    .moves = {MOVE_BODY_SLAM, MOVE_REST, MOVE_SLEEP_TALK, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Timothy5[] = {
    {
    .iv = 240,
    .lvl = 42,
    .species = SPECIES_SNORLAX,
    .moves = {MOVE_BODY_SLAM, MOVE_REST, MOVE_SLEEP_TALK, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Vicky[] = {
    {
    .iv = 200,
    .lvl = 18,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_HIGH_JUMP_KICK, MOVE_MEDITATE, MOVE_CONFUSION, MOVE_DETECT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby1[] = {
    {
    .iv = 200,
    .lvl = 21,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 200,
    .lvl = 21,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby2[] = {
    {
    .iv = 210,
    .lvl = 30,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 210,
    .lvl = 30,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby3[] = {
    {
    .iv = 220,
    .lvl = 33,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 220,
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby4[] = {
    {
    .iv = 230,
    .lvl = 36,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 230,
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shelby5[] = {
    {
    .iv = 240,
    .lvl = 39,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 240,
    .lvl = 39,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin1[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_NICKIT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Billy[] = {
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_YUNGOOS,
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_PANCHAM,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Josh[] = {
    {
    .iv = 100,
    .lvl = 10,
    .species = SPECIES_TAILLOW,
    .moves = {MOVE_QUICK_ATTACK, MOVE_PECK, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tommy[] = {
    {
    .iv = 110,
    .lvl = 8,
    .species = SPECIES_FLETCHLING,
    },
    {
    .iv = 120,
    .lvl = 8,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Joey[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Ben[] = {
    {
    .iv = 150,
    .lvl = 23,
    .species = SPECIES_HELIOPTILE,
    .moves = {MOVE_THUNDER_WAVE, MOVE_PARABOLIC_CHARGE, MOVE_QUICK_ATTACK, MOVE_MUD_SLAP}
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_LITLEO,
    .moves = {MOVE_WILL_O_WISP, MOVE_FIRE_FANG, MOVE_TAKE_DOWN, MOVE_NOBLE_ROAR}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Quincy[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_ORANGURU,
    .moves = {MOVE_PSYCHIC, MOVE_NASTY_PLOT, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_GALLADE,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_PSYCHO_CUT, MOVE_LEAF_BLADE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Katelynn[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_BEHEEYEM,
    .moves = {MOVE_PSYCHIC_TERRAIN, MOVE_PSYCHIC, MOVE_RECOVER, MOVE_CALM_MIND}
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_MEOWSTIC,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_PSYCHIC, MOVE_FAKE_OUT, MOVE_SIGNAL_BEAM}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jaylen[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SIMIPOUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dillon[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_THIEVUL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_HAWLUCHA,
    },
    {
    .iv = 20,
    .lvl = 30,
    .species = SPECIES_THIEVUL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_HAWLUCHA,
    },
    {
    .iv = 30,
    .lvl = 29,
    .species = SPECIES_LINOONE_GALARIAN,
    },
    {
    .iv = 30,
    .lvl = 33,
    .species = SPECIES_THIEVUL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Calvin5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_HAWLUCHA,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_OBSTAGOON,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_THIEVUL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Eddie[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_HONEDGE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Allen[] = {
    {
    .iv = 0,
    .lvl = 4,
    .species = SPECIES_RATTATA_ALOLAN,
    },
    {
    .iv = 0,
    .lvl = 3,
    .species = SPECIES_FLETCHLING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Timmy[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_SUDOWOODO,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wallace[] = {
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_CUSTAP_BERRY,
    .abilityNums = 1,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_EXPLOSION}
    },
    {
    .iv = 255,
    .lvl = 70,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 1,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_HIDDEN_POWER, MOVE_VOLT_SWITCH}
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 2,
    .moves = {MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_FOCUS_BLAST, MOVE_ENERGY_BALL}
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_DOUBLE_EDGE, MOVE_DRAGON_CLAW}
    },
    {
    .iv = 255,
    .lvl = 71,
    .species = SPECIES_ENTEI,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_FLARE_BLITZ, MOVE_CRUNCH, MOVE_EXTREME_SPEED, MOVE_STONE_EDGE}
    },
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_TEA,
    .abilityNums = 2,
    .moves = {MOVE_LIQUIDATION, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andrew[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_WISHIWASHI_SCHOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ivan[] = {
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_WISHIWASHI,
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_FINNEON,
    },
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Claude[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_SEISMITOAD,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_OMASTAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot1[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_BASCULIN_WHITE_STRIPED,
    },
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_POLIWAG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ned[] = {
    {
    .iv = 10,
    .lvl = 5,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dale[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_BARRASKEWDA,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_EMPOLEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nolan[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_CRAMORANT,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_ARAQUANID,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_KABUTOPS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Barny[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_BINACLE,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wade[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_CHINCHOU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carter[] = {
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_FRILLISH,
    },
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot2[] = {
    {
    .iv = 10,
    .lvl = 15,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 10,
    .lvl = 17,
    .species = SPECIES_POLIWHIRL,
    },
    {
    .iv = 10,
    .lvl = 17,
    .species = SPECIES_BASCULIN_WHITE_STRIPED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_POLIWHIRL,
    },
    {
    .iv = 20,
    .lvl = 26,
    .species = SPECIES_BARRASKEWDA,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 30,
    .lvl = 30,
    .species = SPECIES_BARRASKEWDA,
    },
    {
    .iv = 31,
    .lvl = 31,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Elliot5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_BARRASKEWDA,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ronald[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_SLOWKING,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_BARBARACLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jacob[] = {
    {
    .iv = 20,
    .lvl = 46,
    .species = SPECIES_URSARING,
    },
    {
    .iv = 20,
    .lvl = 46,
    .species = SPECIES_QUAGSIRE,
    },
    {
    .iv = 20,
    .lvl = 47,
    .species = SPECIES_EXCADRILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Anthony[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_AVALUGG,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_BEARTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin1[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_PILOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin2[] = {
    {
    .iv = 10,
    .lvl = 52,
    .species = SPECIES_PILOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin3[] = {
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin4[] = {
    {
    .iv = 30,
    .lvl = 56,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benjamin5[] = {
    {
    .iv = 40,
    .lvl = 58,
    .species = SPECIES_MAMOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail1[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_MR_MIME_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jasmine[] = {
    {
    .iv = 80,
    .lvl = 48,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 80,
    .lvl = 48,
    .species = SPECIES_SANDSLASH_ALOLAN,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_NINETALES_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail2[] = {
    {
    .iv = 10,
    .lvl = 52,
    .species = SPECIES_MR_RIME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail3[] = {
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_MR_RIME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail4[] = {
    {
    .iv = 30,
    .lvl = 56,
    .species = SPECIES_MR_RIME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Abigail5[] = {
    {
    .iv = 40,
    .lvl = 58,
    .species = SPECIES_MR_RIME,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dylan5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_PONYTA_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .species = SPECIES_PONYTA_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .species = SPECIES_PONYTA_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_PONYTA_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Maria5[] = {
    {
    .iv = 40,
    .lvl = 42,
    .species = SPECIES_RAPIDASH_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Camden[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_BEARTIC,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_MR_MIME_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Demetrius[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_SCRAFTY,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah1[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo1[] = {
    {
    .iv = 50,
    .lvl = 68,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 50,
    .lvl = 68,
    .species = SPECIES_SLOWBRO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chase[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_GLALIE,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_DELIBIRD,
    },
    {
    .iv = 80,
    .lvl = 46,
    .species = SPECIES_SANDSLASH_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah2[] = {
    {
    .iv = 150,
    .lvl = 70,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 150,
    .lvl = 70,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah3[] = {
    {
    .iv = 200,
    .lvl = 72,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 200,
    .lvl = 72,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah4[] = {
    {
    .iv = 250,
    .lvl = 74,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 250,
    .lvl = 74,
    .species = SPECIES_SWAMPERT_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaiah5[] = {
    {
    .iv = 250,
    .lvl = 76,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 250,
    .lvl = 76,
    .species = SPECIES_SWAMPERT_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isobel[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_KABUTOPS,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_LANTURN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Donny[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_EISCUE,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_URSARING,
    },
    {
    .iv = 160,
    .lvl = 46,
    .species = SPECIES_NINETALES_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Talia[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn1[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Allison[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SAWSBUCK_WINTER,
    },
    {
    .iv = 20,
    .lvl = 46,
    .species = SPECIES_FROSMOTH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn2[] = {
    {
    .iv = 20,
    .lvl = 69,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 20,
    .lvl = 70,
    .species = SPECIES_LUMINEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn3[] = {
    {
    .iv = 40,
    .lvl = 71,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 40,
    .lvl = 72,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 40,
    .lvl = 72,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn4[] = {
    {
    .iv = 60,
    .lvl = 73,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 60,
    .lvl = 74,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 60,
    .lvl = 74,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katelyn5[] = {
    {
    .iv = 80,
    .lvl = 75,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 80,
    .lvl = 76,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 80,
    .lvl = 76,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas1[] = {
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 100,
    .lvl = 65,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas2[] = {
    {
    .iv = 110,
    .lvl = 66,
    .species = SPECIES_LUCARIO,
    },
    {
    .iv = 110,
    .lvl = 67,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas3[] = {
    {
    .iv = 120,
    .lvl = 68,
    .species = SPECIES_LUCARIO,
    },
    {
    .iv = 120,
    .lvl = 69,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicolas4[] = {
    {
    .iv = 130,
    .lvl = 70,
    .species = SPECIES_LUCARIO,
    },
    {
    .iv = 130,
    .lvl = 70,
    .species = SPECIES_HITMONTOP,
    },
    {
    .iv = 130,
    .lvl = 71,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Nicolas5[] = {
    {
    .iv = 140,
    .lvl = 72,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_LIFE_ORB
    },
    {
    .iv = 140,
    .lvl = 72,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_EXPERT_BELT
    },
    {
    .iv = 140,
    .lvl = 73,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_LEFTOVERS
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Aaron[] = {
    {
    .iv = 255,
    .lvl = 39,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_BLAZE_KICK, MOVE_FAKE_OUT, MOVE_STONE_EDGE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Perry[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_TRUMBEAK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hugh[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_EELEKTROSS,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Phil[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_ZEBSTRIKA,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_BRONZONG,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_GALLADE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jared[] = {
    {
    .iv = 100,
    .lvl = 42,
    .species = SPECIES_HAWLUCHA,
    },
    {
    .iv = 100,
    .lvl = 43,
    .species = SPECIES_HITMONTOP,
    },
    {
    .iv = 100,
    .lvl = 43,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Humberto[] = {
    {
    .iv = 250,
    .lvl = 43,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 250,
    .lvl = 43,
    .species = SPECIES_GURDURR,
    },
    {
    .iv = 250,
    .lvl = 44,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Presley[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_CRAMORANT,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwardo[] = {
    {
    .iv = 150,
    .lvl = 42,
    .species = SPECIES_GRAPPLOCT,
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_PRIMEAPE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Colin[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_TORTERRA,
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_NIDOQUEEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert1[] = {
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_XATU,
    },
    {
    .iv = 50,
    .lvl = 62,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Benny[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chester[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert2[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_XATU,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert3[] = {
    {
    .iv = 150,
    .lvl = 65,
    .species = SPECIES_XATU,
    },
    {
    .iv = 150,
    .lvl = 66,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert4[] = {
    {
    .iv = 200,
    .lvl = 67,
    .species = SPECIES_XATU,
    },
    {
    .iv = 200,
    .lvl = 68,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robert5[] = {
    {
    .iv = 250,
    .lvl = 69,
    .species = SPECIES_XATU,
    },
    {
    .iv = 250,
    .lvl = 70,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alex[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_FEAROW,
    },
    {
    .iv = 150,
    .lvl = 39,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beck[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Yasu[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_LIEPARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Takashi[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_SNEASEL,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_FLETCHINDER,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Dianne[] = {
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_HURRICANE, MOVE_HYDRO_PUMP, MOVE_U_TURN, MOVE_TAILWIND}
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_SCALD, MOVE_VOLT_SWITCH, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jani[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_SPIRITOMB,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_PYROAR,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_SWALOT,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao1[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_HEAT_WAVE, MOVE_SLUDGE_BOMB, MOVE_CLEAR_SMOG, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_JAB, MOVE_HAZE, MOVE_GLARE, MOVE_THIEF}
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_TOXICROAK,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_POISON_JAB, MOVE_LOW_KICK, MOVE_ASTONISH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lung[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SNEASEL,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao2[] = {
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_HEAT_WAVE, MOVE_SLUDGE_BOMB, MOVE_CLEAR_SMOG, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_JAB, MOVE_COIL, MOVE_GLARE, MOVE_CRUNCH}
    },
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_SNEASEL_HISUIAN,
    .moves = {MOVE_HONE_CLAWS, MOVE_POISON_JAB, MOVE_BRICK_BREAK, MOVE_METAL_CLAW}
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_TOXICROAK,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SLUDGE_BOMB, MOVE_LOW_KICK, MOVE_NASTY_PLOT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao3[] = {
    {
    .iv = 20,
    .lvl = 47,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_HEAT_WAVE, MOVE_SLUDGE_BOMB, MOVE_CLEAR_SMOG, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 20,
    .lvl = 47,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_JAB, MOVE_COIL, MOVE_GLARE, MOVE_CRUNCH}
    },
    {
    .iv = 20,
    .lvl = 47,
    .species = SPECIES_TOXICROAK,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SLUDGE_BOMB, MOVE_LOW_KICK, MOVE_NASTY_PLOT}
    },
    {
    .iv = 20,
    .lvl = 49,
    .species = SPECIES_SNEASLER,
    .moves = {MOVE_HONE_CLAWS, MOVE_POISON_JAB, MOVE_BRICK_BREAK, MOVE_METAL_CLAW}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lao4[] = {
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_HEAT_WAVE, MOVE_SLUDGE_BOMB, MOVE_CLEAR_SMOG, MOVE_EXPLOSION}
    },
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_JAB, MOVE_COIL, MOVE_GLARE, MOVE_CRUNCH}
    },
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_TOXICROAK,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SLUDGE_BOMB, MOVE_LOW_KICK, MOVE_NASTY_PLOT}
    },
    {
    .iv = 30,
    .lvl = 50,
    .species = SPECIES_SNEASLER,
    .moves = {MOVE_HONE_CLAWS, MOVE_POISON_JAB, MOVE_BRICK_BREAK, MOVE_METAL_CLAW}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Lao5[] = {
    {
    .iv = 40,
    .lvl = 53,
    .species = SPECIES_WEEZING,
    .moves = {MOVE_HEAT_WAVE, MOVE_SLUDGE_BOMB, MOVE_CLEAR_SMOG, MOVE_EXPLOSION}
    },
    {
    .iv = 40,
    .lvl = 53,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_JAB, MOVE_COIL, MOVE_GLARE, MOVE_CRUNCH}
    },
    {
    .iv = 40,
    .lvl = 53,
    .species = SPECIES_TOXICROAK,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SLUDGE_BOMB, MOVE_LOW_KICK, MOVE_NASTY_PLOT}
    },
    {
    .iv = 40,
    .lvl = 53,
    .species = SPECIES_SNEASLER,
    .moves = {MOVE_HONE_CLAWS, MOVE_POISON_JAB, MOVE_BRICK_BREAK, MOVE_METAL_CLAW}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jocelyn[] = {
    {
    .iv = 127,
    .lvl = 16,
    .species = SPECIES_DEDENNE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Laura[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_CARBINK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy1[] = {
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cora[] = {
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_MURKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paula[] = {
    {
    .iv = 100,
    .lvl = 26,
    .species = SPECIES_ARBOK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 110,
    .lvl = 26,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 120,
    .lvl = 29,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 130,
    .lvl = 32,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cyndy5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_ROSERADE,
    },
    {
    .iv = 140,
    .lvl = 35,
    .species = SPECIES_SCEPTILE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline1[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GOTHORITA,
    .moves = {MOVE_PSYSHOCK, MOVE_PSYBEAM, MOVE_TICKLE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clarissa[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_HATTERENE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Angelica[] = {
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_VIKAVOLT,
    .moves = {MOVE_AGILITY, MOVE_THUNDERBOLT, MOVE_BUG_BUZZ, MOVE_ACROBATICS}
    },
    {
    .iv = 50,
    .lvl = 62,
    .species = SPECIES_BRAVIARY,
    .moves = {MOVE_BRAVE_BIRD, MOVE_SUPERPOWER, MOVE_WHIRLWIND, MOVE_SLASH}
    },
    {
    .iv = 50,
    .lvl = 62,
    .species = SPECIES_BLISSEY,
    .moves = {MOVE_SOFT_BOILED, MOVE_SEISMIC_TOSS, MOVE_THUNDER_WAVE, MOVE_DAZZLING_GLEAM}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .species = SPECIES_GOTHORITA,
    .moves = {MOVE_PSYCHIC, MOVE_PSYSHOCK, MOVE_TICKLE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline3[] = {
    {
    .iv = 20,
    .lvl = 42,
    .species = SPECIES_GOTHITELLE,
    .moves = {MOVE_PSYCHIC, MOVE_PSYSHOCK, MOVE_TICKLE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline4[] = {
    {
    .iv = 30,
    .lvl = 44,
    .species = SPECIES_HATTERENE,
    .moves = {MOVE_CALM_MIND, MOVE_DAZZLING_GLEAM, MOVE_PSYBEAM, MOVE_BRUTAL_SWING}
    },
    {
    .iv = 30,
    .lvl = 44,
    .species = SPECIES_GOTHITELLE,
    .moves = {MOVE_PSYCHIC, MOVE_PSYSHOCK, MOVE_TICKLE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Madeline5[] = {
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_HATTERENE,
    .moves = {MOVE_CALM_MIND, MOVE_DAZZLING_GLEAM, MOVE_PSYCHIC, MOVE_BRUTAL_SWING}
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_GOTHITELLE,
    .moves = {MOVE_PSYCHIC, MOVE_PSYSHOCK, MOVE_TICKLE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beverly[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_PACHIRISU,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_TOGEDEMARU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Imani[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_SEEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kyla[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_PURRLOIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Denise[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_GLAMEOW,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_CORSOLA,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_JYNX,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_MUDBRAY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Beth[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tara[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_MORELULL,
    },
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Missy[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_MIENFOO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alice[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_DEDENNE,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_DIGGERSBY,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny1[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Grace[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_DEWGONG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tanya[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sharon[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SWANNA,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_CRAWDAUNT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nikki[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brenda[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_FLOATZEL,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Katie[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Susie[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kara[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_CARRACOSTA,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_LAPRAS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dana[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sienna[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_GASTRODON_EAST_SEA,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Debra[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_CRAMORANT,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_BASCULEGION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Linda[] = {
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_POLITOED,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_TOXAPEX,
    },
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kaylee[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Laurel[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 0,
    .lvl = 39,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carlee[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_BRUXISH,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny2[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 39,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny3[] = {
    {
    .iv = 0,
    .lvl = 41,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny4[] = {
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenny5[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_MANTINE,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SLOWKING,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Heidi[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_SHIINOTIC,
    .moves = {MOVE_SPORE, MOVE_MOONBLAST, MOVE_GIGA_DRAIN, MOVE_STRENGTH_SAP}
    },
    {
    .iv = 150,
    .lvl = 72,
    .species = SPECIES_LEAFEON,
    .moves = {MOVE_SWORDS_DANCE, MOVE_LEAF_BLADE, MOVE_X_SCISSOR, MOVE_AERIAL_ACE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Becky[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_TANGROWTH,
    .moves = {MOVE_POWER_WHIP, MOVE_KNOCK_OFF, MOVE_SLEEP_POWDER, MOVE_SLAM}
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_AZUMARILL,
    .moves = {MOVE_PLAY_ROUGH, MOVE_AQUA_JET, MOVE_AQUA_TAIL, MOVE_ROLLOUT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carol[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SOLOSIS,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_GOTHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nancy[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_LOPUNNY,
    },
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_MUSHARNA,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Martha[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_KROKOROK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana1[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_ARCANINE,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Cedric[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_VICTREEBEL,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_SYNTHESIS, MOVE_GIGA_DRAIN, MOVE_TICKLE}
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_NOCTOWL,
    .moves = {MOVE_EXTRASENSORY, MOVE_HYPNOSIS, MOVE_SHADOW_BALL, MOVE_AERIAL_ACE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Irene[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_BISHARP,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_CORVIKNIGHT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana2[] = {
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 20,
    .lvl = 54,
    .species = SPECIES_ARCANINE,
    },
    {
    .iv = 20,
    .lvl = 55,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana3[] = {
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_ARCANINE,
    },
    {
    .iv = 40,
    .lvl = 57,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana4[] = {
    {
    .iv = 60,
    .lvl = 58,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 60,
    .lvl = 58,
    .species = SPECIES_ARCANINE,
    },
    {
    .iv = 60,
    .lvl = 59,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Diana5[] = {
    {
    .iv = 80,
    .lvl = 60,
    .species = SPECIES_AMBIPOM,
    },
    {
    .iv = 80,
    .lvl = 60,
    .species = SPECIES_ARCANINE,
    },
    {
    .iv = 80,
    .lvl = 61,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GinaAndMia1[] = {
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_COTTONEE,
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_PETILIL,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MiuAndYuki[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_VANILLUXE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_BLIZZARD, MOVE_FREEZE_DRY, MOVE_ICE_SHARD, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_GLALITITE,
    .abilityNums = 1,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_CRUNCH, MOVE_EARTHQUAKE, MOVE_FREEZE_DRY}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv3[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_GinaAndMia2[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_COTTONEE,
    .moves = {MOVE_LEECH_SEED, MOVE_FAIRY_WIND, MOVE_TICKLE, MOVE_ABSORB}
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_PETILIL,
    .moves = {MOVE_ABSORB, MOVE_LEECH_SEED, MOVE_SLEEP_POWDER, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_AmyAndLiv4[] = {
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AmyAndLiv5[] = {
    {
    .iv = 30,
    .lvl = 43,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 30,
    .lvl = 43,
    .species = SPECIES_MINUN,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_AmyAndLiv6[] = {
    {
    .iv = 40,
    .lvl = 46,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 40,
    .lvl = 46,
    .species = SPECIES_MINUN,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Huey[] = {
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_BINACLE,
    },
    {
    .iv = 10,
    .lvl = 32,
    .species = SPECIES_DHELMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edmond[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_EXEGGUTOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest1[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dwayne[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Phillip[] = {
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 0,
    .lvl = 44,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leonard[] = {
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 43,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Duncan[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_ARCANINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 10,
    .lvl = 39,
    .species = SPECIES_CLAWITZER,
    },
    {
    .iv = 10,
    .lvl = 40,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest3[] = {
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_CLAWITZER,
    },
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest4[] = {
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_CLAWITZER,
    },
    {
    .iv = 30,
    .lvl = 42,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ernest5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_LUMINEON,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_CLAWITZER,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Eli[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_QUAGSIRE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Annika[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_ZWEILOUS,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_BODY_SLAM, MOVE_DRAGON_PULSE, MOVE_ROAR, MOVE_CRUNCH}
    },
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_FLAPPLE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_GRAV_APPLE, MOVE_DRAGON_RUSH, MOVE_ACROBATICS, MOVE_U_TURN}
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_HYDREIGON,
    .heldItem = ITEM_ORAN_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_TRI_ATTACK, MOVE_OUTRAGE, MOVE_BODY_SLAM, MOVE_CRUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Jazmyn[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_COMFEY,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_DRAINING_KISS, MOVE_GIGA_DRAIN, MOVE_CALM_MIND, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_MAWILITE,
    .abilityNums = 1,
    .moves = {MOVE_PLAY_ROUGH, MOVE_SUCKER_PUNCH, MOVE_BRICK_BREAK, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Jonas[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_FLAME_ORB,
    .abilityNums = 1,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_KNOCK_OFF, MOVE_HEAVY_SLAM, MOVE_STONE_EDGE}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_MEDICHAMITE,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_HIGH_JUMP_KICK, MOVE_ICE_PUNCH, MOVE_ZEN_HEADBUTT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Kayley[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_BODY_SLAM, MOVE_LAVA_PLUME, MOVE_SNORE, MOVE_PROTECT}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_CAMERUPTITE,
    .abilityNums = 1,
    .moves = {MOVE_FIRE_BLAST, MOVE_EARTH_POWER, MOVE_REST, MOVE_SLEEP_TALK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Auron[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_BOLTUND,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kelvin[] = {
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 150,
    .lvl = 39,
    .species = SPECIES_THROH,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Marley[] = {
    {
    .iv = 255,
    .lvl = 39,
    .species = SPECIES_HITMONCHAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 1,
    .moves = {MOVE_MACH_PUNCH, MOVE_CLOSE_COMBAT, MOVE_THUNDER_PUNCH, MOVE_ICE_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Reyna[] = {
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_BEWEAR,
    },
    {
    .iv = 200,
    .lvl = 39,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hudson[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_FLOATZEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Conor[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 200,
    .lvl = 33,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin1[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_KLANG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hector[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_BEAUTIFLY,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_DUSTOX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_TabithaMossdeep[] = {
    {
    .iv = 100,
    .lvl = 36,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 100,
    .lvl = 38,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 100,
    .lvl = 40,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin2[] = {
    {
    .iv = 0,
    .lvl = 51,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_KLINKLANG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin3[] = {
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 0,
    .lvl = 54,
    .species = SPECIES_KLINKLANG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin4[] = {
    {
    .iv = 0,
    .lvl = 55,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 0,
    .lvl = 56,
    .species = SPECIES_KLINKLANG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Edwin5[] = {
    {
    .iv = 0,
    .lvl = 57,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 0,
    .lvl = 58,
    .species = SPECIES_KLINKLANG,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_WallyVR1[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 41,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 45,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRoute103Mudkip[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_GRIMER,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute110Mudkip[] = {
    {
    .iv = 100,
    .lvl = 18,
    .species = SPECIES_FLAAFFY,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_THUNDER_WAVE, MOVE_CHARGE, MOVE_TACKLE}
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_QUAGSIRE,
    .moves = {MOVE_MUD_SHOT, MOVE_WATER_GUN, MOVE_SLAM, MOVE_MUD_BOMB}
    },
    {
    .iv = 100,
    .lvl = 21,
    .species = SPECIES_QUILAVA,
    .moves = {MOVE_FLAME_WHEEL, MOVE_QUICK_ATTACK, MOVE_SMOKESCREEN, MOVE_LEER}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute119Mudkip[] = {
    {
    .iv = 150,
    .lvl = 31,
    .species = SPECIES_ELECTRODE,
    .moves = {MOVE_SELF_DESTRUCT, MOVE_THUNDERBOLT, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 150,
    .lvl = 31,
    .species = SPECIES_MAROWAK,
    .moves = {MOVE_THRASH, MOVE_BONEMERANG, MOVE_ICE_BEAM, MOVE_THUNDERBOLT}
    },
    {
    .iv = 150,
    .lvl = 32,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_FLASH_CANNON, MOVE_METAL_SOUND, MOVE_ELECTRO_BALL, MOVE_THUNDERBOLT}
    },
    {
    .iv = 150,
    .lvl = 32,
    .species = SPECIES_NIDOKING,
    .moves = {MOVE_EARTHQUAKE, MOVE_POISON_JAB, MOVE_SUBSTITUTE, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute103Treecko[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_BITE, MOVE_EMBER, MOVE_ROAR, MOVE_SMOG}
    },
    {
    .iv = 150,
    .lvl = 23,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_PAYBACK, MOVE_ROLLOUT, MOVE_FAKE_OUT}
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_SHARPEDO,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_SWAGGER, MOVE_ASSURANCE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute110Treecko[] = {
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_BITE, MOVE_EMBER, MOVE_ROAR, MOVE_SMOG}
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_MEGA_DRAIN, MOVE_PAYBACK, MOVE_ROLLOUT, MOVE_FAKE_OUT}
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_THIEVUL,
    .moves = {MOVE_NASTY_PLOT, MOVE_SNARL, MOVE_ASSURANCE, MOVE_QUICK_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_SHARPEDO,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_SWAGGER, MOVE_ASSURANCE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute119Treecko[] = {
    {
    .iv = 150,
    .lvl = 34,
    .species = SPECIES_VAPOREON,
    .moves = {MOVE_SCALD, MOVE_TOXIC, MOVE_WISH, MOVE_PROTECT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute103Torchic[] = {
    {
    .iv = 150,
    .lvl = 34,
    .species = SPECIES_FLAREON,
    .moves = {MOVE_CURSE, MOVE_QUICK_ATTACK, MOVE_FIRE_FANG, MOVE_FIRE_SPIN}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute110Torchic[] = {
    {
    .iv = 150,
    .lvl = 34,
    .species = SPECIES_JOLTEON,
    .moves = {MOVE_THUNDERBOLT, MOVE_HIDDEN_POWER, MOVE_DOUBLE_TEAM, MOVE_DOUBLE_KICK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_BrendanRoute119Torchic[] = {
    {
    .iv = 150,
    .lvl = 34,
    .species = SPECIES_ESPEON,
    .moves = {MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_MORNING_SUN}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_MayRoute103Mudkip[] = {
    {
    .iv = 150,
    .lvl = 34,
    .species = SPECIES_UMBREON,
    .moves = {MOVE_WISH, MOVE_PROTECT, MOVE_DARK_PULSE, MOVE_CONFUSE_RAY}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_MayRoute110Mudkip[] = {
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_KROOKODILE,
    .moves = {MOVE_FOUL_PLAY, MOVE_DIG, MOVE_SWAGGER, MOVE_GRASS_KNOT}
    },
    {
    .iv = 150,
    .lvl = 45,
    .species = SPECIES_MALAMAR,
    .moves = {MOVE_FOUL_PLAY, MOVE_PSYCHO_CUT, MOVE_PLUCK, MOVE_REFLECT}
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_LEAF_STORM, MOVE_KNOCK_OFF, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 150,
    .lvl = 45,
    .species = SPECIES_SHARPEDO,
    .moves = {MOVE_AQUA_JET, MOVE_ICE_FANG, MOVE_POISON_FANG, MOVE_CRUNCH}
    },
    {
    .iv = 150,
    .lvl = 46,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_FOUL_PLAY, MOVE_FLAMETHROWER, MOVE_THUNDER_FANG, MOVE_HOWL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute119Mudkip[] = {
    {
    .iv = 250,
    .lvl = 46,
    .species = SPECIES_NOIVERN,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_HURRICANE, MOVE_U_TURN, MOVE_TAUNT, MOVE_ROOST}
    },
    {
    .iv = 250,
    .lvl = 46,
    .species = SPECIES_SCOLIPEDE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 0,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_POISON_JAB}
    },
    {
    .iv = 250,
    .lvl = 47,
    .species = SPECIES_MAROWAK_ALOLAN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_SHADOW_BONE, MOVE_FLARE_BLITZ, MOVE_EARTHQUAKE, MOVE_BRICK_BREAK}
    },
    {
    .iv = 250,
    .lvl = 47,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 2,
    .moves = {MOVE_POISON_JAB, MOVE_X_SCISSOR, MOVE_BRICK_BREAK, MOVE_FAKE_OUT}
    },
    {
    .iv = 250,
    .lvl = 48,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_SHELL_BELL,
    .abilityNums = 0,
    .moves = {MOVE_NASTY_PLOT, MOVE_PSYCHIC, MOVE_HYPNOSIS, MOVE_DAZZLING_GLEAM}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute103Treecko[] = {
    {
    .iv = 250,
    .lvl = 54,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 2,
    .moves = {MOVE_EXPLOSION, MOVE_THUNDERBOLT, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 250,
    .lvl = 54,
    .species = SPECIES_RHYDON,
    .heldItem = ITEM_EVIOLITE,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 250,
    .lvl = 54,
    .species = SPECIES_WEAVILE,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_KNOCK_OFF, MOVE_ICICLE_CRASH, MOVE_ICE_SHARD, MOVE_PURSUIT}
    },
    {
    .iv = 250,
    .lvl = 55,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 1,
    .moves = {MOVE_FLASH_CANNON, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER, MOVE_THUNDERBOLT}
    },
    {
    .iv = 250,
    .lvl = 55,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_EARTH_POWER, MOVE_SLUDGE_WAVE, MOVE_MEGAHORN, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute110Treecko[] = {
    {
    .iv = 250,
    .lvl = 63,
    .species = SPECIES_GALVANTULA,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_STICKY_WEB, MOVE_VOLT_SWITCH, MOVE_BUG_BUZZ, MOVE_THUNDER}
    },
    {
    .iv = 250,
    .lvl = 63,
    .species = SPECIES_KROOKODILE,
    .heldItem = ITEM_QUICK_CLAW,
    .abilityNums = 0,
    .moves = {MOVE_FOUL_PLAY, MOVE_EARTHQUAKE, MOVE_GRASS_KNOT, MOVE_STONE_EDGE}
    },
    {
    .iv = 250,
    .lvl = 64,
    .species = SPECIES_SHARPEDO,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 2,
    .moves = {MOVE_PROTECT, MOVE_WATERFALL, MOVE_CRUNCH, MOVE_EARTHQUAKE}
    },
    {
    .iv = 250,
    .lvl = 64,
    .species = SPECIES_MUK_ALOLAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_KNOCK_OFF, MOVE_SHADOW_SNEAK, MOVE_GUNK_SHOT, MOVE_FIRE_BLAST}
    },
    {
    .iv = 250,
    .lvl = 64,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 2,
    .moves = {MOVE_AERIAL_ACE, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_IRON_HEAD}
    },
    {
    .iv = 250,
    .lvl = 65,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 1,
    .moves = {MOVE_HIDDEN_POWER, MOVE_FIRE_BLAST, MOVE_SLUDGE_BOMB, MOVE_DARK_PULSE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRoute119Treecko[] = {
    {
    .iv = 255,
    .lvl = 72,
    .species = SPECIES_GALVANTULA,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_STICKY_WEB, MOVE_VOLT_SWITCH, MOVE_BUG_BUZZ, MOVE_THUNDER}
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .abilityNums = 2,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_ROOST, MOVE_FACADE}
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_SHARPEDO,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 2,
    .moves = {MOVE_ICE_FANG, MOVE_WATERFALL, MOVE_CRUNCH, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 73,
    .species = SPECIES_MUK_ALOLAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_KNOCK_OFF, MOVE_SHADOW_SNEAK, MOVE_GUNK_SHOT, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 74,
    .species = SPECIES_MEGANIUM,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_SUBSTITUTE, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 74,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_HOUNDOOMINITE,
    .abilityNums = 1,
    .moves = {MOVE_NASTY_PLOT, MOVE_FIRE_BLAST, MOVE_SLUDGE_BOMB, MOVE_DARK_PULSE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute103Torchic[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute110Torchic[] = {
    {
    .iv = 50,
    .lvl = 18,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 50,
    .lvl = 18,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRoute119Torchic[] = {
    {
    .iv = 100,
    .lvl = 29,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 100,
    .lvl = 29,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 150,
    .lvl = 31,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_NATU,
    },
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_WOOLOO,
    },
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_ARON,
    },
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_ROCKRUFF,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_MONFERNO,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Davis[] = {
    {
    .iv = 250,
    .lvl = 77,
    .species = SPECIES_HIPPOWDON,
    .heldItem = ITEM_ROCKY_HELMET,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_STEALTH_ROCK, MOVE_SLACK_OFF, MOVE_WHIRLWIND}
    },
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_TYRANITAR,
    .heldItem = ITEM_TYRANITARITE,
    .abilityNums = 0,
    .moves = {MOVE_STONE_EDGE, MOVE_EARTHQUAKE, MOVE_CRUNCH, MOVE_THUNDER_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Mitchell[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_LYCANROC,
    .moves = {MOVE_STONE_EDGE, MOVE_ACCELEROCK, MOVE_SWORDS_DANCE, MOVE_FIRE_FANG}
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_PORYGON_Z,
    .moves = {MOVE_TRI_ATTACK, MOVE_THUNDERBOLT, MOVE_ICE_BEAM, MOVE_NASTY_PLOT}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_NATU,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_WOOLOO,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_ARON,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_ROCKRUFF,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_MONFERNO,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_XATU,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_ARON,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_LYCANROC,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_MONFERNO,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_XATU,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_ARON,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_LYCANROC,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_MONFERNO,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isaac5[] = {
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_XATU,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_LYCANROC,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_MONFERNO,
    },
    {
    .iv = 40,
    .lvl = 31,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_SPRITZEE,
    },
    {
    .iv = 0,
    .lvl = 17,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_VULPIX,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_PRINPLUP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Halle[] = {
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_BIBAREL,
    },
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_ARBOK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Garrison[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_GARBODOR,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_DRAPION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_PIKACHU,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = 10,
    .lvl = 22,
    .species = SPECIES_PRINPLUP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_RAICHU,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = 20,
    .lvl = 25,
    .species = SPECIES_PRINPLUP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_RAICHU,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = 30,
    .lvl = 28,
    .species = SPECIES_PRINPLUP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lydia5[] = {
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_RAICHU,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_GROVYLE,
    },
    {
    .iv = 40,
    .lvl = 32,
    .species = SPECIES_PRINPLUP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson1[] = {
    {
    .iv = 50,
    .lvl = 60,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_SHIINOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lorenzo[] = {
    {
    .iv = 150,
    .lvl = 61,
    .species = SPECIES_CORVIKNIGHT,
    },
    {
    .iv = 150,
    .lvl = 62,
    .species = SPECIES_AERODACTYL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sebastian[] = {
    {
    .iv = 50,
    .lvl = 39,
    .species = SPECIES_CACTURNE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson2[] = {
    {
    .iv = 100,
    .lvl = 65,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = 100,
    .lvl = 66,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 100,
    .lvl = 66,
    .species = SPECIES_SHIINOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson3[] = {
    {
    .iv = 150,
    .lvl = 68,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = 150,
    .lvl = 69,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 150,
    .lvl = 69,
    .species = SPECIES_SHIINOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson4[] = {
    {
    .iv = 200,
    .lvl = 70,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = 200,
    .lvl = 71,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 200,
    .lvl = 71,
    .species = SPECIES_SHIINOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jackson5[] = {
    {
    .iv = 250,
    .lvl = 73,
    .species = SPECIES_WHIMSICOTT,
    },
    {
    .iv = 250,
    .lvl = 74,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 250,
    .lvl = 74,
    .species = SPECIES_SHIINOTIC,
    },
    {
    .iv = 250,
    .lvl = 74,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine1[] = {
    {
    .iv = 50,
    .lvl = 60,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 50,
    .lvl = 61,
    .species = SPECIES_MIMIKYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jenna[] = {
    {
    .iv = 150,
    .lvl = 61,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 150,
    .lvl = 62,
    .species = SPECIES_MUDSDALE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sophia[] = {
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 50,
    .lvl = 38,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine2[] = {
    {
    .iv = 100,
    .lvl = 63,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 100,
    .lvl = 64,
    .species = SPECIES_MIMIKYU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine3[] = {
    {
    .iv = 150,
    .lvl = 66,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 150,
    .lvl = 67,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 150,
    .lvl = 67,
    .species = SPECIES_MIMIKYU,
    },
    {
    .iv = 150,
    .lvl = 67,
    .species = SPECIES_MISMAGIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine4[] = {
    {
    .iv = 200,
    .lvl = 68,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 200,
    .lvl = 69,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 200,
    .lvl = 69,
    .species = SPECIES_MIMIKYU,
    },
    {
    .iv = 200,
    .lvl = 69,
    .species = SPECIES_MISMAGIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Catherine5[] = {
    {
    .iv = 250,
    .lvl = 70,
    .species = SPECIES_GOURGEIST,
    },
    {
    .iv = 250,
    .lvl = 71,
    .species = SPECIES_CURSOLA,
    },
    {
    .iv = 250,
    .lvl = 71,
    .species = SPECIES_MIMIKYU,
    },
    {
    .iv = 250,
    .lvl = 71,
    .species = SPECIES_MISMAGIUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Julio[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_CRYOGONAL,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_DARMANITAN_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSeafloorCavern5[] = {
    {
    .iv = 50,
    .lvl = 35,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 50,
    .lvl = 35,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntUnused[] = {
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtPyre4[] = {
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 30,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntJaggedPass[] = {
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marc[] = {
    {
    .iv = 120,
    .lvl = 8,
    .species = SPECIES_PIKIPEK,
    },
    {
    .iv = 130,
    .lvl = 8,
    .species = SPECIES_HOPPIP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brenden[] = {
    {
    .iv = 100,
    .lvl = 16,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lilith[] = {
    {
    .iv = 150,
    .lvl = 16,
    .species = SPECIES_IMPIDIMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristian[] = {
    {
    .iv = 200,
    .lvl = 16,
    .species = SPECIES_COMFEY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sylvia[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_SANDSLASH_ALOLAN,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_NINETALES_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leonardo[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_RELICANTH,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_POLIWRATH,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Athena[] = {
    {
    .iv = 100,
    .lvl = 45,
    .species = SPECIES_CRYOGONAL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_RECOVER, MOVE_CONFUSE_RAY, MOVE_ICE_BEAM, MOVE_REFLECT}
    },
    {
    .iv = 100,
    .lvl = 46,
    .species = SPECIES_FURRET,
    .heldItem = ITEM_QUICK_CLAW,
    .abilityNums = 0,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_SLAM, MOVE_COIL, MOVE_U_TURN}
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_JYNX,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 2,
    .moves = {MOVE_BLIZZARD, MOVE_DRAINING_KISS, MOVE_BODY_SLAM, MOVE_PSYCHIC}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Harrison[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_QUAGSIRE,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_JELLICENT,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_HUNTAIL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMtChimney2[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_PERSIAN_ALOLAN,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_DRAGALGE,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_HONCHKROW,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clarence[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_AVALUGG_HISUIAN,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_PILOSWINE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Terry[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nate[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_PILOSWINE,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_JYNX,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_ROTOM_FROST,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kathleen[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_GLALIE,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_AVALUGG,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_AURORUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clifford[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_BEARTIC,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Nicholas[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_FROSMOTH,
    },
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_JYNX,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_AVALUGG_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter3[] = {
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 31,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter4[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter5[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter6[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntSpaceCenter7[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Macey[] = {
    {
    .iv = 100,
    .lvl = 47,
    .species = SPECIES_VANILLUXE,
    },
    {
    .iv = 100,
    .lvl = 48,
    .species = SPECIES_CLOYSTER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRustboroTreecko[] = {
    {
    .iv = 25,
    .lvl = 13,
    .species = SPECIES_LOTAD,
    },
    {
    .iv = 50,
    .lvl = 15,
    .species = SPECIES_TORCHIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRustboroMudkip[] = {
    {
    .iv = 25,
    .lvl = 13,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Paxton[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isabella[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_MILOTIC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntWeatherInst5[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_RATTATA_ALOLAN,
    },
    {
    .iv = 0,
    .lvl = 14,
    .species = SPECIES_LITWICK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_TabithaMtChimney[] = {
    {
    .iv = 50,
    .lvl = 18,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 50,
    .lvl = 20,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 50,
    .lvl = 22,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jonathan[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_LYCANROC_MIDNIGHT,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_EXEGGUTOR,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_EMPOLEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanRustboroTorchic[] = {
    {
    .iv = 25,
    .lvl = 13,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 50,
    .lvl = 15,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRustboroMudkip[] = {
    {
    .iv = 25,
    .lvl = 13,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MaxieMagmaHideout[] = {
    {
    .iv = 150,
    .lvl = 37,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 38,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 150,
    .lvl = 39,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MaxieMtChimney[] = {
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 24,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 150,
    .lvl = 25,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tiana[] = {
    {
    .iv = 0,
    .lvl = 4,
    .species = SPECIES_BUNNELBY,
    },
    {
    .iv = 0,
    .lvl = 4,
    .species = SPECIES_ODDISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley1[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_CUTIEFLY,
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_PIKACHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Janice[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Vivi[] = {
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 100,
    .lvl = 15,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_PIKACHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_RAICHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 30,
    .lvl = 32,
    .species = SPECIES_RAICHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Haley5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 40,
    .lvl = 35,
    .species = SPECIES_ALCREMIE,
    },
    {
    .iv = 40,
    .lvl = 34,
    .species = SPECIES_RAICHU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sally[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_GLOOM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Robin[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SKRELP,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_PHANTUMP,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_LINOONE_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andrea[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_TYRANTRUM,
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Crissy[] = {
    {
    .iv = 100,
    .lvl = 55,
    .species = SPECIES_FLYGON,
    },
    {
    .iv = 100,
    .lvl = 56,
    .species = SPECIES_DRAGALGE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rick[] = {
    {
    .iv = 0,
    .lvl = 4,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 0,
    .lvl = 4,
    .species = SPECIES_GRUBBIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lyle[] = {
    {
    .iv = 0,
    .lvl = 3,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 0,
    .lvl = 3,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 0,
    .lvl = 3,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 0,
    .lvl = 3,
    .species = SPECIES_WURMPLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jose[] = {
    {
    .iv = 50,
    .lvl = 11,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 50,
    .lvl = 12,
    .species = SPECIES_DRILBUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Doug[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_KOFFING,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_RATICATE_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Greg[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_GRIMER_ALOLAN,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_CARKOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kent[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_PAWNIARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James1[] = {
    {
    .iv = 0,
    .lvl = 16,
    .species = SPECIES_BUTTERFREE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .species = SPECIES_BEEDRILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_BUTTERFREE,
    },
    {
    .iv = 10,
    .lvl = 26,
    .species = SPECIES_BEEDRILL,
    },
    {
    .iv = 10,
    .lvl = 27,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_BUTTERFREE,
    },
    {
    .iv = 20,
    .lvl = 28,
    .species = SPECIES_BEEDRILL,
    },
    {
    .iv = 20,
    .lvl = 29,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_VIKAVOLT,
    },
    {
    .iv = 30,
    .lvl = 31,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_James5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_MASQUERAIN,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_SCIZOR,
    },
    {
    .iv = 40,
    .lvl = 33,
    .species = SPECIES_VIKAVOLT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brice[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_FLAAFFY,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_PONYTA_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent1[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_MUDBRAY,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_TAUROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lenny[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_STONJOURNER,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_ARMALDO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lucas1[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_POLIWRATH,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_LYCANROC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alan[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_RATICATE,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_TRUBBISH,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_CARKOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Clark[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_SHUCKLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Eric[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_STONJOURNER,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_RHYDON,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Lucas2[] = {
    {
    .iv = 0,
    .lvl = 9,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_SPLASH, MOVE_WATER_GUN, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Mike1[] = {
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_GUST, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 10,
    .species = SPECIES_POOCHYENA,
    .moves = {MOVE_BITE, MOVE_SCARY_FACE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mike2[] = {
    {
    .iv = 0,
    .lvl = 16,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 16,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 16,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_TAUROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent3[] = {
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 20,
    .lvl = 34,
    .species = SPECIES_TAUROS,
    },
    {
    .iv = 20,
    .lvl = 35,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_TAUROS,
    },
    {
    .iv = 30,
    .lvl = 37,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Trent5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_DUBWOOL,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_TAUROS,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_AMPHAROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_DezAndLuke[] = {
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 0,
    .lvl = 7,
    .species = SPECIES_TIMBURR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LeaAndJed[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan1[] = {
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 100,
    .lvl = 74,
    .species = SPECIES_WEEZING,
    },
    {
    .iv = 100,
    .lvl = 75,
    .species = SPECIES_MUK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_KiraAndDan5[] = {
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 40,
    .lvl = 39,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Johanna[] = {
    {
    .iv = 0,
    .lvl = 13,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Gerald[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_KINGLER,
    .moves = {MOVE_METAL_CLAW, MOVE_MUD_SHOT, MOVE_SCALD, MOVE_STOMP}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Vivian[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_TAUROS,
    .moves = {MOVE_PAYBACK, MOVE_HORN_ATTACK, MOVE_SCARY_FACE, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_DUBWOOL,
    .moves = {MOVE_THUNDER_WAVE, MOVE_STOMP, MOVE_HEADBUTT, MOVE_DOUBLE_KICK}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Danielle[] = {
    {
    .iv = 100,
    .lvl = 30,
    .species = SPECIES_GOLISOPOD,
    .moves = {MOVE_RAZOR_SHELL, MOVE_SLASH, MOVE_BUG_BITE, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Hideo[] = {
    {
    .iv = 20,
    .lvl = 21,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SELF_DESTRUCT, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    },
    {
    .iv = 20,
    .lvl = 21,
    .species = SPECIES_MONFERNO,
    .moves = {MOVE_FLAME_WHEEL, MOVE_MACH_PUNCH, MOVE_TAUNT, MOVE_FURY_SWIPES}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Keigo[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_VICTREEBEL,
    .moves = {MOVE_LEAF_STORM, MOVE_SLEEP_POWDER, MOVE_SLUDGE_BOMB, MOVE_KNOCK_OFF}
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_SWALOT,
    .moves = {MOVE_YAWN, MOVE_GUNK_SHOT, MOVE_AMNESIA, MOVE_BODY_SLAM}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Riley[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_VENOMOTH,
    .moves = {MOVE_QUIVER_DANCE, MOVE_BUG_BUZZ, MOVE_PSYCHIC, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_SKUNTANK,
    .moves = {MOVE_EXPLOSION, MOVE_FLAMETHROWER, MOVE_SUCKER_PUNCH, MOVE_TOXIC}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Flint[] = {
    {
    .iv = 150,
    .lvl = 42,
    .species = SPECIES_SAWK,
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_CRABOMINABLE,
    },
    {
    .iv = 150,
    .lvl = 43,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ashley[] = {
    {
    .iv = 100,
    .lvl = 43,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 100,
    .lvl = 43,
    .species = SPECIES_FALINKS,
    },
    {
    .iv = 100,
    .lvl = 43,
    .species = SPECIES_GALLADE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_WallyMauville[] = {
    {
    .iv = 30,
    .lvl = 16,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_WallyVR2[] = {
    {
    .iv = 150,
    .lvl = 47,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 46,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 47,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 44,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 48,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_WallyVR3[] = {
    {
    .iv = 150,
    .lvl = 50,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 49,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 50,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 47,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 51,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_WallyVR4[] = {
    {
    .iv = 150,
    .lvl = 53,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 52,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 53,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 50,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 54,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_WallyVR5[] = {
    {
    .iv = 150,
    .lvl = 56,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 55,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 56,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 53,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 57,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveMudkip[] = {
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_KABUTOPS,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_WATERFALL, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_SUPERPOWER}
    },
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_TOXICROAK,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_SWORDS_DANCE, MOVE_GUNK_SHOT, MOVE_DRAIN_PUNCH, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_BLASTOISE,
    .heldItem = ITEM_BLASTOISINITE,
    .abilityNums = 2,
    .moves = {MOVE_RAPID_SPIN, MOVE_WATER_PULSE, MOVE_ICE_BEAM, MOVE_DARK_PULSE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveTreecko[] = {
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_GASTRODON,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_EARTH_POWER, MOVE_ICE_BEAM, MOVE_RECOVER, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_HELIOLISK,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 0,
    .moves = {MOVE_VOLT_SWITCH, MOVE_HYPER_VOICE, MOVE_THUNDER, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_GOODRA,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_PULSE, MOVE_SLUDGE_BOMB, MOVE_THUNDER, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonItemCustomMoves sParty_BrendanLilycoveTorchic[] = {
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_GALVANTULA,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_STICKY_WEB, MOVE_VOLT_SWITCH, MOVE_BUG_BUZZ, MOVE_THUNDER}
    },
    {
    .iv = 255,
    .lvl = 77,
    .species = SPECIES_MEGANIUM,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_SUBSTITUTE, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_HOUNDOOMINITE,
    .abilityNums = 1,
    .moves = {MOVE_NASTY_PLOT, MOVE_FIRE_BLAST, MOVE_SLUDGE_BOMB, MOVE_DARK_PULSE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayLilycoveMudkip[] = {
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 2,
    .moves = {MOVE_EXPLOSION, MOVE_THUNDERBOLT, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER}
    },
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_RHYDON,
    .heldItem = ITEM_EVIOLITE,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_SWORDS_DANCE}
    },
    {
    .iv = 255,
    .lvl = 79,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_RAPID_SPIN, MOVE_SLUDGE_BOMB, MOVE_KNOCK_OFF}
    },
    {
    .iv = 255,
    .lvl = 79,
    .species = SPECIES_WEAVILE,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_KNOCK_OFF, MOVE_ICICLE_CRASH, MOVE_ICE_SHARD, MOVE_PURSUIT}
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_MAGNEZONE,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 1,
    .moves = {MOVE_FLASH_CANNON, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER, MOVE_THUNDERBOLT}
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_NIDOKING,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_EARTH_POWER, MOVE_SLUDGE_WAVE, MOVE_MEGAHORN, MOVE_ICE_BEAM}
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayLilycoveTreecko[] = {
    {
    .iv = 255,
    .lvl = 78,
    .species = SPECIES_ALAKAZAM,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 2,
    .moves = {MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_FOCUS_BLAST, MOVE_ENERGY_BALL}
    },
    {
    .iv = 255,
    .lvl = 79,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_DOUBLE_EDGE, MOVE_DRAGON_CLAW}
    },
    {
    .iv = 255,
    .lvl = 80,
    .species = SPECIES_FERALIGATR,
    .heldItem = ITEM_TEA,
    .abilityNums = 2,
    .moves = {MOVE_LIQUIDATION, MOVE_DRAGON_DANCE, MOVE_ICE_PUNCH, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonDynamicLevel sParty_MayLilycoveTorchic[] = {
    {
    .iv = 0,
    .species = SPECIES_MILTANK,
    .moves = {MOVE_MEMENTO, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jonah[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_DEWGONG,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Henry[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_KINGLER,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Roger[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SEAKING,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alexa[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_ALOMOMOLA,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Ruben[] = {
    {
    .iv = 100,
    .lvl = 67,
    .species = SPECIES_TSAREENA,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_EXEGGUTOR_ALOLAN,
    },
    {
    .iv = 100,
    .lvl = 68,
    .species = SPECIES_BLASTOISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji1[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_BEWEAR,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_CRABOMINABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wayne[] = {
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 0,
    .lvl = 67,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 0,
    .lvl = 68,
    .species = SPECIES_CARRACOSTA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Aidan[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LAMPENT,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_DARMANITAN,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_BLAZIKEN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Reed[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_URSARING,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SEALEO,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_VANILLISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tisha[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_CLOYSTER,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_ABOMASNOW,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_WALREIN,
    },
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_AURORUS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ToriAndTia[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_SEALEO,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_PILOSWINE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_KimAndIris[] = {
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_EMOLGA,
    .moves = {MOVE_ACROBATICS, MOVE_ENCORE, MOVE_THUNDERBOLT, MOVE_NUZZLE}
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_STUNFISK,
    .moves = {MOVE_MUD_BOMB, MOVE_DISCHARGE, MOVE_BOUNCE, MOVE_BIDE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_TyraAndIvy[] = {
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_HIPPOWDON,
    .moves = {MOVE_EARTHQUAKE, MOVE_THUNDER_FANG, MOVE_YAWN, MOVE_CRUNCH}
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_SKARMORY,
    .moves = {MOVE_STEEL_WING, MOVE_NIGHT_SLASH, MOVE_AIR_SLASH, MOVE_AUTOTOMIZE}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_MelAndPaul[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_TOGETIC,
    .moves = {MOVE_FOLLOW_ME, MOVE_ENCORE, MOVE_WISH, MOVE_DRAINING_KISS}
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_PERSIAN_ALOLAN,
    .moves = {MOVE_NASTY_PLOT, MOVE_FAKE_OUT, MOVE_DARK_PULSE, MOVE_POWER_GEM}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay1[] = {
    {
    .iv = 200,
    .lvl = 64,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_BLAZE_KICK, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT}
    },
    {
    .iv = 200,
    .lvl = 64,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT, MOVE_DRAIN_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay2[] = {
    {
    .iv = 210,
    .lvl = 66,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_BLAZE_KICK, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT}
    },
    {
    .iv = 210,
    .lvl = 66,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT, MOVE_DRAIN_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay3[] = {
    {
    .iv = 220,
    .lvl = 68,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_BLAZE_KICK, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT}
    },
    {
    .iv = 220,
    .lvl = 68,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT, MOVE_DRAIN_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay4[] = {
    {
    .iv = 230,
    .lvl = 70,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_BLAZE_KICK, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT}
    },
    {
    .iv = 230,
    .lvl = 70,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT, MOVE_DRAIN_PUNCH}
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_JohnAndJay5[] = {
    {
    .iv = 240,
    .lvl = 72,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_SUCKER_PUNCH, MOVE_BLAZE_KICK, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT}
    },
    {
    .iv = 240,
    .lvl = 72,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT, MOVE_DRAIN_PUNCH}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_ReliAndIan[] = {
    {
    .iv = 200,
    .lvl = 70,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 200,
    .lvl = 70,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy1[] = {
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy2[] = {
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 0,
    .lvl = 42,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy3[] = {
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 0,
    .lvl = 45,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy4[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LilaAndRoy5[] = {
    {
    .iv = 0,
    .lvl = 51,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 0,
    .lvl = 51,
    .species = SPECIES_SWAMPERT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_LisaAndRay[] = {
    {
    .iv = 0,
    .lvl = 24,
    .species = SPECIES_GLIGAR,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chris[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_ALOMOMOLA,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_HUNTAIL,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_RELICANTH,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Dawson[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_VENOMOTH,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_DRUDDIGON,
    .heldItem = ITEM_NONE
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Sarah[] = {
    {
    .iv = 0,
    .lvl = 36,
    .species = SPECIES_SIMISEAR,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_CENTISKORCH,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darian[] = {
    {
    .iv = 0,
    .lvl = 11,
    .species = SPECIES_QWILFISH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hailey[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_COMBUSKEN,
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_BOUFFALANT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Chandler[] = {
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_MUNNA,
    },
    {
    .iv = 0,
    .lvl = 34,
    .species = SPECIES_SWOOBAT,
    }
};

static const struct TrainerMonItemDefaultMoves sParty_Kaleb[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_MUDSDALE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Joseph[] = {
    {
    .iv = 0,
    .lvl = 48,
    .species = SPECIES_SIMIPOUR,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_EXEGGUTOR_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alyssa[] = {
    {
    .iv = 0,
    .lvl = 46,
    .species = SPECIES_FROSMOTH,
    },
    {
    .iv = 0,
    .lvl = 47,
    .species = SPECIES_WALREIN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marcos[] = {
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_TOXTRICITY_LOW_KEY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rhett[] = {
    {
    .iv = 100,
    .lvl = 33,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tyron[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_HEATMOR,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_CHANDELURE,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Celina[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_ARBOK,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_OBSTAGOON,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_TOXAPEX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bianca[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SEVIPER,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_LIEPARD,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_SPIRITOMB,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Hayden[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_TYRANITAR,
    },
    {
    .iv = 100,
    .lvl = 73,
    .species = SPECIES_KROOKODILE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sophie[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SANDYGAST,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_ORANGURU,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Coby[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_AVALUGG,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lawrence[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_ORICORIO_PAU,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_KOMALA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Wyatt[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_CLAUNCHER,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_EXEGGCUTE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Angelina[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_GIGALITH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kai[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_QUAGSIRE,
    },
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_DREDNAW,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_TOXAPEX,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Charlotte[] = {
    {
    .iv = 0,
    .lvl = 52,
    .species = SPECIES_CARNIVINE,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_TANGROWTH,
    },
    {
    .iv = 0,
    .lvl = 53,
    .species = SPECIES_CRADILY,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Deandre[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_DEERLING_AUTUMN,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_NIDORINO,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_TAUROS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout1[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_GOLBAT,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_MAGMAR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout2[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout3[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_CENTISKORCH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout4[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_HONCHKROW,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_ARCANINE_HISUIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout5[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_RATICATE,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_RATICATE_ALOLAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout6[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_GARBODOR,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_COALOSSAL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout7[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_HEATMOR,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout8[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_DARMANITAN,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_MUK,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout9[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_HOUNDOOM,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_LINOONE_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout10[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_CRAWDAUNT,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_SLOWBRO_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout11[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_BISHARP,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_AMOONGUSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout12[] = {
    {
    .iv = 200,
    .lvl = 51,
    .species = SPECIES_VENOMOTH,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_COALOSSAL,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_THIEVUL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout13[] = {
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_ZOROARK,
    },
    {
    .iv = 200,
    .lvl = 52,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 200,
    .lvl = 53,
    .species = SPECIES_WEEZING,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout14[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_LIEPARD,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_SALAZZLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout15[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_SCRAFTY,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_PYROAR,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_WEAVILE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_GruntMagmaHideout16[] = {
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_QWILFISH_HISUIAN,
    },
    {
    .iv = 0,
    .lvl = 49,
    .species = SPECIES_CHANDELURE,
    },
    {
    .iv = 0,
    .lvl = 50,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_TabithaMagmaHideout[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_GRIMMSNARL,
    .heldItem = ITEM_LIGHT_CLAY,
    .abilityNums = 0,
    .moves = {MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_TAUNT, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_SCOLIPEDE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_MEGAHORN, MOVE_POISON_JAB}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 2,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_POISON_JAB, MOVE_U_TURN, MOVE_X_SCISSOR}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darcy[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_PORYGON_Z,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_ELECTIVIRE,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MaxieMossdeep[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_HONCHKROW,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_BRAVE_BIRD, MOVE_SUCKER_PUNCH, MOVE_SUPERPOWER, MOVE_ROOST}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_VILEPLUME,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 2,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_SLEEP_POWDER, MOVE_MOONBLAST}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_HYDREIGON,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_DARK_PULSE, MOVE_FIRE_BLAST, MOVE_U_TURN, MOVE_OUTRAGE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pete[] = {
    {
    .iv = 0,
    .lvl = 15,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Isabelle[] = {
    {
    .iv = 0,
    .lvl = 15,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres1[] = {
    {
    .iv = 50,
    .lvl = 24,
    .species = SPECIES_MUDBRAY,
    },
    {
    .iv = 50,
    .lvl = 25,
    .species = SPECIES_BOLDORE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Josue[] = {
    {
    .iv = 50,
    .lvl = 24,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 50,
    .lvl = 25,
    .species = SPECIES_TRANQUILL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Camron[] = {
    {
    .iv = 0,
    .lvl = 6,
    .species = SPECIES_LITLEO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory1[] = {
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_TYROGUE,
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 0,
    .lvl = 12,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Carolina[] = {
    {
    .iv = 50,
    .lvl = 12,
    .species = SPECIES_BUIZEL,
    },
    {
    .iv = 50,
    .lvl = 12,
    .species = SPECIES_MIME_JR,
    },
    {
    .iv = 50,
    .lvl = 13,
    .species = SPECIES_SNUBBULL,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Elijah[] = {
    {
    .iv = 0,
    .lvl = 32,
    .species = SPECIES_EELEKTRIK,
    .moves = {MOVE_DISCHARGE, MOVE_ACID, MOVE_BIND, MOVE_HEADBUTT}
    },
    {
    .iv = 0,
    .lvl = 33,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_DISCHARGE, MOVE_BITE, MOVE_THUNDER_FANG, MOVE_QUICK_ATTACK}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Celia[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_SAWSBUCK_SUMMER,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_GOGOAT,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_AMOONGUSS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bryan[] = {
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_DRAPION,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Branden[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_FLAPPLE,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ELECTRODE_HISUIAN,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_FERROTHORN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Bryant[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_VULLABY,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_RUFFLET,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Shayla[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_HERDIER,
    },
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_BOLTUND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Kyra[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_CHARJABUG,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Jaiden[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_ACCELGOR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alix[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_STARAVIA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Helene[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_LEAFEON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Marlene[] = {
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_WORMADAM,
    },
    {
    .iv = 0,
    .lvl = 21,
    .species = SPECIES_DURANT,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Devan[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GOGOAT,
    },
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_CHARIZARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Johnson[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_PROBOPASS,
    },
    {
    .iv = 0,
    .lvl = 38,
    .species = SPECIES_BLASTOISE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Melina[] = {
    {
    .iv = 0,
    .lvl = 19,
    .species = SPECIES_SCYTHER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brandi[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_GARDEVOIR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Aisha[] = {
    {
    .iv = 0,
    .lvl = 37,
    .species = SPECIES_LUCARIO,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Makayla[] = {
    {
    .iv = 100,
    .lvl = 69,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fabian[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_BARBARACLE,
    },
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_JELLICENT,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_ARAQUANID,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Dayton[] = {
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 0,
    .lvl = 20,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Rachel[] = {
    {
    .iv = 100,
    .lvl = 60,
    .species = SPECIES_ORBEETLE,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMonNoItemCustomMoves sParty_Leonel[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_TOXAPEX,
    .moves = {MOVE_BANEFUL_BUNKER, MOVE_TOXIC, MOVE_POISON_JAB, MOVE_LIQUIDATION}
    },
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_CRUSTLE,
    .moves = {MOVE_SHELL_SMASH, MOVE_ROCK_BLAST, MOVE_X_SCISSOR, MOVE_EARTHQUAKE}
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_RAPIDASH,
    .moves = {MOVE_FLARE_BLITZ, MOVE_BOUNCE, MOVE_HIGH_HORSEPOWER, MOVE_MEGAHORN}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Callie[] = {
    {
    .iv = 100,
    .lvl = 61,
    .species = SPECIES_EMOLGA,
    },
    {
    .iv = 100,
    .lvl = 62,
    .species = SPECIES_CHARIZARD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cale[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_PINSIR,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_BEAUTIFLY,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_ACCELGOR,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ESCAVALIER,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_FORRETRESS,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Myles[] = {
    {
    .iv = 250,
    .lvl = 71,
    .species = SPECIES_RATICATE,
    .heldItem = ITEM_FLAME_ORB,
    .abilityNums = 1,
    .moves = {MOVE_FACADE, MOVE_CRUNCH, MOVE_U_TURN, MOVE_SUPER_FANG}
    },
    {
    .iv = 250,
    .lvl = 72,
    .species = SPECIES_RATICATE_ALOLAN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SUCKER_PUNCH, MOVE_KNOCK_OFF, MOVE_DOUBLE_EDGE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pat[] = {
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_ALCREMIE,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_RIBOMBEE,
    },
    {
    .iv = 100,
    .lvl = 72,
    .species = SPECIES_RAPIDASH_GALARIAN,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin1[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 100,
    .lvl = 71,
    .species = SPECIES_GOTHITELLE,
    }
};

static const struct TrainerMonItemCustomMoves sParty_MayRustboroTreecko[] = {
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_KROOKODILE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_STONE_EDGE, MOVE_KNOCK_OFF, MOVE_STEALTH_ROCK}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_MALAMAR,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_SUPERPOWER, MOVE_KNOCK_OFF, MOVE_PSYCHO_CUT, MOVE_PLUCK}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_SHELL_BELL,
    .abilityNums = 0,
    .moves = {MOVE_NASTY_PLOT, MOVE_FLAMETHROWER, MOVE_DARK_PULSE, MOVE_SLUDGE_BOMB}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayRustboroTorchic[] = {
    {
    .iv = 25,
    .lvl = 13,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne2[] = {
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_MASQUERAIN,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 0,
    .moves = {MOVE_STICKY_WEB, MOVE_BUG_BUZZ, MOVE_AIR_SLASH, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .abilityNums = 2,
    .moves = {MOVE_SWORDS_DANCE, MOVE_EARTHQUAKE, MOVE_ROOST, MOVE_ICE_FANG}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_STARAPTOR,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 2,
    .moves = {MOVE_BRAVE_BIRD, MOVE_CLOSE_COMBAT, MOVE_DOUBLE_EDGE, MOVE_U_TURN}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_DRAGON_DANCE, MOVE_ROOST, MOVE_EARTHQUAKE, MOVE_DRAGON_CLAW}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_HAWLUCHA,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 1,
    .moves = {MOVE_ACROBATICS, MOVE_HIGH_JUMP_KICK, MOVE_SWORDS_DANCE, MOVE_ROOST}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_PIDGEOT,
    .heldItem = ITEM_PIDGEOTITE,
    .abilityNums = 2,
    .moves = {MOVE_HURRICANE, MOVE_HEAT_WAVE, MOVE_U_TURN, MOVE_ROOST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne3[] = {
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_OVERQWIL,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 2,
    .moves = {MOVE_TOXIC_SPIKES, MOVE_DESTINY_BOND, MOVE_CRUNCH, MOVE_GUNK_SHOT}
    },
    {
    .iv = 255,
    .lvl = 86,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_SCALD, MOVE_RAPID_SPIN, MOVE_SLUDGE_BOMB, MOVE_KNOCK_OFF}
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_MUK_ALOLAN,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 0,
    .moves = {MOVE_CURSE, MOVE_SHADOW_SNEAK, MOVE_POISON_JAB, MOVE_KNOCK_OFF}
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_SLOWBRO_GALARIAN,
    .heldItem = ITEM_QUICK_CLAW,
    .abilityNums = 0,
    .moves = {MOVE_SHELL_SIDE_ARM, MOVE_PSYSHOCK, MOVE_ICE_BEAM, MOVE_SCALD}
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_NIDOQUEEN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_EARTH_POWER, MOVE_THUNDERBOLT, MOVE_SLUDGE_WAVE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_VENUSAUR,
    .heldItem = ITEM_VENUSAURITE,
    .abilityNums = 0,
    .moves = {MOVE_LEECH_SEED, MOVE_POWER_WHIP, MOVE_SYNTHESIS, MOVE_EARTH_POWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne4[] = {
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_ROTOM_HEAT,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_SHADOW_BALL, MOVE_OVERHEAT, MOVE_VOLT_SWITCH, MOVE_PAIN_SPLIT}
    },
    {
    .iv = 255,
    .lvl = 87,
    .species = SPECIES_DARMANITAN,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_FLARE_BLITZ, MOVE_EARTHQUAKE, MOVE_U_TURN, MOVE_GRASS_KNOT}
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_TURTONATOR,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_SHELL_SMASH, MOVE_FIRE_BLAST, MOVE_DRAGON_PULSE, MOVE_SOLAR_BEAM}
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_VOLCARONA,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_QUIVER_DANCE, MOVE_BUG_BUZZ, MOVE_FLAMETHROWER, MOVE_ROOST}
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_ARCANINE_HISUIAN,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 2,
    .moves = {MOVE_HEAD_SMASH, MOVE_FLARE_BLITZ, MOVE_EXTREME_SPEED, MOVE_WILD_CHARGE}
    },
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_Y,
    .abilityNums = 0,
    .moves = {MOVE_FIRE_BLAST, MOVE_SOLAR_BEAM, MOVE_FOCUS_BLAST, MOVE_ROOST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Roxanne5[] = {
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_PALOSSAND,
    .heldItem = ITEM_ROCKY_HELMET,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_SHADOW_BALL, MOVE_EARTH_POWER, MOVE_SHORE_UP}
    },
    {
    .iv = 255,
    .lvl = 88,
    .species = SPECIES_MIMIKYU,
    .heldItem = ITEM_RED_CARD,
    .abilityNums = 0,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SHADOW_CLAW, MOVE_SHADOW_SNEAK, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_SPIRITOMB,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_SHADOW_SNEAK, MOVE_DARK_PULSE, MOVE_PSYCHIC, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_DRAGAPULT,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_DRAGON_DANCE, MOVE_DRAGON_DARTS, MOVE_PHANTOM_FORCE, MOVE_SUCKER_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_AEGISLASH,
    .heldItem = ITEM_IRON_BALL,
    .abilityNums = 0,
    .moves = {MOVE_KINGS_SHIELD, MOVE_TOXIC, MOVE_GYRO_BALL, MOVE_SHADOW_BALL}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_GENGAR,
    .heldItem = ITEM_GENGARITE,
    .abilityNums = 0,
    .moves = {MOVE_SHADOW_BALL, MOVE_SLUDGE_WAVE, MOVE_THUNDERBOLT, MOVE_FOCUS_BLAST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_GRIMMSNARL,
    .heldItem = ITEM_LIGHT_CLAY,
    .abilityNums = 0,
    .moves = {MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_TAUNT, MOVE_DRAIN_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_KLEFKI,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 0,
    .moves = {MOVE_SPIKES, MOVE_THUNDER_WAVE, MOVE_FOUL_PLAY, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_HATTERENE,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_CALM_MIND, MOVE_DRAINING_KISS, MOVE_PSYSHOCK, MOVE_MYSTICAL_FIRE}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 1,
    .moves = {MOVE_BELLY_DRUM, MOVE_PLAY_ROUGH, MOVE_KNOCK_OFF, MOVE_AQUA_JET}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_RAPIDASH_GALARIAN,
    .heldItem = ITEM_SHELL_BELL,
    .abilityNums = 1,
    .moves = {MOVE_SWORDS_DANCE, MOVE_PLAY_ROUGH, MOVE_HIGH_HORSEPOWER, MOVE_MORNING_SUN}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_MAWILITE,
    .abilityNums = 1,
    .moves = {MOVE_PLAY_ROUGH, MOVE_THUNDER_PUNCH, MOVE_ICE_PUNCH, MOVE_SUCKER_PUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly3[] = {
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_ROCKY_HELMET,
    .abilityNums = 1,
    .moves = {MOVE_ROOST, MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_BRAVE_BIRD}
    },
    {
    .iv = 255,
    .lvl = 89,
    .species = SPECIES_EXCADRILL,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 0,
    .moves = {MOVE_DRILL_RUN, MOVE_IRON_HEAD, MOVE_SWORDS_DANCE, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_BRONZONG,
    .heldItem = ITEM_RED_CARD,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_GYRO_BALL, MOVE_PSYCHIC, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_FERROTHORN,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_KNOCK_OFF, MOVE_POWER_WHIP, MOVE_GYRO_BALL, MOVE_LEECH_SEED}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_EMPOLEON,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_SWORDS_DANCE, MOVE_AQUA_JET, MOVE_DRILL_PECK, MOVE_LIQUIDATION}
    },
    {
    .iv = 255,
    .lvl = 91,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_METAGROSSITE,
    .abilityNums = 0,
    .moves = {MOVE_METEOR_MASH, MOVE_STONE_EDGE, MOVE_ZEN_HEADBUTT, MOVE_BULLET_PUNCH}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly4[] = {
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 2,
    .moves = {MOVE_SHELL_SMASH, MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_EARTH_POWER}
    },
    {
    .iv = 255,
    .lvl = 90,
    .species = SPECIES_RIBOMBEE,
    .heldItem = ITEM_FOCUS_SASH,
    .abilityNums = 0,
    .moves = {MOVE_STICKY_WEB, MOVE_MOONBLAST, MOVE_BUG_BUZZ, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 91,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_CURSE, MOVE_BODY_SLAM, MOVE_HIGH_HORSEPOWER, MOVE_ZEN_HEADBUTT}
    },
    {
    .iv = 255,
    .lvl = 91,
    .species = SPECIES_CHARIZARD,
    .heldItem = ITEM_CHARIZARDITE_X,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_FLARE_BLITZ, MOVE_DRAGON_CLAW, MOVE_ROOST}
    },
    {
    .iv = 255,
    .lvl = 92,
    .species = SPECIES_MEWTWO,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_PSYSTRIKE, MOVE_FOCUS_BLAST, MOVE_ICE_BEAM, MOVE_THUNDERBOLT}
    },
    {
    .iv = 255,
    .lvl = 100,
    .species = SPECIES_PIKACHU,
    .heldItem = ITEM_LIGHT_BALL,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_THUNDERBOLT, MOVE_SURF, MOVE_GRASS_KNOT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Brawly5[] = {
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_HITMONLEE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_MEGA_KICK, MOVE_FOCUS_PUNCH, MOVE_EARTHQUAKE, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_HITMONCHAN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SKY_UPPERCUT, MOVE_PROTECT, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_CROSS_CHOP, MOVE_ROCK_SLIDE, MOVE_FOCUS_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_PURSUIT, MOVE_COUNTER, MOVE_PROTECT, MOVE_TRIPLE_KICK}
    },
    {
    .iv = 255,
    .lvl = 52,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_PUNCH, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Wattson2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_WIGGLYTUFF,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_STEALTH_ROCK, MOVE_HYPER_VOICE, MOVE_DAZZLING_GLEAM, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_WYRDEER,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_MEGAHORN, MOVE_EARTH_POWER, MOVE_THUNDERBOLT}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_URSALUNA,
    .heldItem = ITEM_FLAME_ORB,
    .abilityNums = 0,
    .moves = {MOVE_HIGH_HORSEPOWER, MOVE_FACADE, MOVE_THUNDER_PUNCH, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_ZOROARK_HISUIAN,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_NASTY_PLOT, MOVE_PSYCHIC, MOVE_SHADOW_BALL, MOVE_HYPER_VOICE}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_GIGA_IMPACT, MOVE_HAMMER_ARM, MOVE_KNOCK_OFF, MOVE_FIRE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_KANGASKHANITE,
    .abilityNums = 1,
    .moves = {MOVE_FAKE_OUT, MOVE_BODY_SLAM, MOVE_ICE_PUNCH, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMonDynamicLevel sParty_Wattson3[] = {
    {
    .iv = 0,
    .species = SPECIES_CHANSEY,
    .moves = {MOVE_MEMENTO, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonDynamicLevel sParty_Wattson4[] = {
    {
    .iv = 0,
    .species = SPECIES_BLISSEY,
    .moves = {MOVE_MEMENTO, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonDynamicLevel sParty_Wattson5[] = {
    {
    .iv = 0,
    .species = SPECIES_CLEFABLE,
    .moves = {MOVE_MEMENTO, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_SEAKING,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_WATERFALL, MOVE_KNOCK_OFF, MOVE_DRILL_RUN, MOVE_MEGAHORN}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_TOXAPEX,
    .heldItem = ITEM_ROCKY_HELMET,
    .abilityNums = 2,
    .moves = {MOVE_SCALD, MOVE_POISON_JAB, MOVE_TOXIC_SPIKES, MOVE_RECOVER}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_CLOYSTER,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 1,
    .moves = {MOVE_ICICLE_SPEAR, MOVE_SHELL_SMASH, MOVE_ROCK_BLAST, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ENERGY_BALL, MOVE_ICE_BEAM, MOVE_FAKE_OUT}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_CRUNCH, MOVE_ICE_FANG, MOVE_WATERFALL}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_SWAMPERT,
    .heldItem = ITEM_SWAMPERTITE,
    .abilityNums = 2,
    .moves = {MOVE_WATERFALL, MOVE_EARTHQUAKE, MOVE_ICE_PUNCH, MOVE_SUPERPOWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery3[] = {
    {
    .iv = 255,
    .lvl = 41,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_ROAR, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .species = SPECIES_PONYTA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery4[] = {
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_HOUNDOUR,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Flannery5[] = {
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .abilityNums = 0,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_ELECTRODE_HISUIAN,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 1,
    .moves = {MOVE_VOLT_SWITCH, MOVE_ENERGY_BALL, MOVE_THUNDERBOLT, MOVE_FOUL_PLAY}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_PINCURCHIN,
    .heldItem = ITEM_TERRAIN_EXTENDER,
    .abilityNums = 2,
    .moves = {MOVE_SPIKES, MOVE_ZING_ZAP, MOVE_SCALD, MOVE_POISON_JAB}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_EELEKTROSS,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_GIGA_DRAIN, MOVE_FLAMETHROWER, MOVE_KNOCK_OFF}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_ROTOM_WASH,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 0,
    .moves = {MOVE_VOLT_SWITCH, MOVE_HYDRO_PUMP, MOVE_PAIN_SPLIT, MOVE_THUNDERBOLT}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_TOXTRICITY,
    .heldItem = ITEM_THROAT_SPRAY,
    .abilityNums = 0,
    .moves = {MOVE_OVERDRIVE, MOVE_BOOMBURST, MOVE_SLUDGE_BOMB, MOVE_VOLT_SWITCH}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_MANECTITE,
    .abilityNums = 0,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLAMETHROWER, MOVE_VOLT_SWITCH, MOVE_HIDDEN_POWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman3[] = {
    {
    .iv = 255,
    .lvl = 47,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 47,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman4[] = {
    {
    .iv = 255,
    .lvl = 52,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 52,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Norman5[] = {
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_PROTECT, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 57,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TAKE_DOWN, MOVE_PROTECT, MOVE_FIRE_BLAST, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_INFERNAPE,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_FLARE_BLITZ, MOVE_STONE_EDGE}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_SNEASLER,
    .heldItem = ITEM_AIR_BALLOON,
    .abilityNums = 1,
    .moves = {MOVE_SWORDS_DANCE, MOVE_CLOSE_COMBAT, MOVE_THROAT_CHOP, MOVE_POISON_JAB}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_SCRAFTY,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_DRAGON_DANCE, MOVE_CRUNCH, MOVE_HIGH_JUMP_KICK, MOVE_STONE_EDGE}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 1,
    .moves = {MOVE_THUNDER_PUNCH, MOVE_DYNAMIC_PUNCH, MOVE_KNOCK_OFF, MOVE_BULLET_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_URSHIFU_RAPID_STRIKE_STYLE,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 0,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_SURGING_STRIKES, MOVE_U_TURN, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_LUCARIO,
    .heldItem = ITEM_LUCARIONITE,
    .abilityNums = 2,
    .moves = {MOVE_SWORDS_DANCE, MOVE_METEOR_MASH, MOVE_BULLET_PUNCH, MOVE_CLOSE_COMBAT}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona3[] = {
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_HOOTHOOT,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 43,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 46,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona4[] = {
    {
    .iv = 255,
    .lvl = 48,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 49,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Winona5[] = {
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_HYPER_BEAM, MOVE_THUNDERBOLT, MOVE_EARTHQUAKE, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_SKY_ATTACK, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_MAMOSWINE,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 2,
    .moves = {MOVE_HIGH_HORSEPOWER, MOVE_ICE_SHARD, MOVE_ICICLE_CRASH, MOVE_STEALTH_ROCK}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_NINETALES_ALOLAN,
    .heldItem = ITEM_LIGHT_CLAY,
    .abilityNums = 2,
    .moves = {MOVE_AURORA_VEIL, MOVE_FREEZE_DRY, MOVE_ICY_WIND, MOVE_DAZZLING_GLEAM}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_BEARTIC,
    .heldItem = ITEM_CHOICE_BAND,
    .abilityNums = 1,
    .moves = {MOVE_ICICLE_CRASH, MOVE_SUPERPOWER, MOVE_AQUA_JET, MOVE_PLAY_ROUGH}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_FREEZE_DRY, MOVE_HYDRO_PUMP, MOVE_BLIZZARD, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_DARMANITAN_GALARIAN,
    .heldItem = ITEM_CHOICE_SCARF,
    .abilityNums = 0,
    .moves = {MOVE_ICICLE_CRASH, MOVE_FLARE_BLITZ, MOVE_U_TURN, MOVE_HAMMER_ARM}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_GLALITITE,
    .abilityNums = 1,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_SHADOW_BALL, MOVE_CRUNCH, MOVE_FREEZE_DRY}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza3[] = {
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_DROWZEE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 54,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza4[] = {
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_TateAndLiza5[] = {
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_SLOWKING,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan2[] = {
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_DRAGALGE,
    .heldItem = ITEM_BLACK_SLUDGE,
    .abilityNums = 2,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_TOXIC_SPIKES, MOVE_HYDRO_PUMP, MOVE_DRAGON_PULSE}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHOICE_SPECS,
    .abilityNums = 2,
    .moves = {MOVE_HYDRO_PUMP, MOVE_DRAGON_PULSE, MOVE_ICE_BEAM, MOVE_FLASH_CANNON}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_GOODRA_HISUIAN,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 2,
    .moves = {MOVE_FLASH_CANNON, MOVE_FIRE_BLAST, MOVE_SCALD, MOVE_DRAGON_PULSE}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 2,
    .moves = {MOVE_DRAGON_DANCE, MOVE_STEEL_WING, MOVE_DRAGON_CLAW, MOVE_THUNDER_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 84,
    .species = SPECIES_LATIAS,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 0,
    .moves = {MOVE_CALM_MIND, MOVE_ICE_BEAM, MOVE_PSYCHIC, MOVE_THUNDERBOLT}
    },
    {
    .iv = 255,
    .lvl = 85,
    .species = SPECIES_GARCHOMP,
    .heldItem = ITEM_GARCHOMPITE,
    .abilityNums = 2,
    .moves = {MOVE_DRAGON_CLAW, MOVE_EARTHQUAKE, MOVE_POISON_JAB, MOVE_STONE_EDGE}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan3[] = {
    {
    .iv = 255,
    .lvl = 50,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 51,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 53,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan4[] = {
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 56,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Juan5[] = {
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 61,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_HYDRO_PUMP, MOVE_PERISH_SONG}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_SHEER_COLD}
    },
    {
    .iv = 255,
    .lvl = 63,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 66,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMonItemCustomMoves sParty_Angelo[] = {
    {
    .iv = 100,
    .lvl = 23,
    .species = SPECIES_FURRET,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_COIL, MOVE_QUICK_ATTACK, MOVE_FURY_SWIPES, MOVE_TOXIC}
    },
    {
    .iv = 100,
    .lvl = 24,
    .species = SPECIES_CINCCINO,
    .heldItem = ITEM_NONE,
    .abilityNums = 0,
    .moves = {MOVE_TAIL_SLAP, MOVE_SING, MOVE_ROCK_BLAST, MOVE_THUNDER_WAVE}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Darius[] = {
    {
    .iv = 200,
    .lvl = 43,
    .species = SPECIES_LILLIGANT_HISUIAN,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_PANGORO,
    },
    {
    .iv = 200,
    .lvl = 44,
    .species = SPECIES_SIRFETCHD,
    }
};

static const struct TrainerMonItemCustomMoves sParty_Steven[] = {
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_CLEFABLE,
    .heldItem = ITEM_LEFTOVERS,
    .abilityNums = 1,
    .moves = {MOVE_STEALTH_ROCK, MOVE_MOONBLAST, MOVE_SOFT_BOILED, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 81,
    .species = SPECIES_SCIZOR,
    .heldItem = ITEM_LIFE_ORB,
    .abilityNums = 1,
    .moves = {MOVE_BULLET_PUNCH, MOVE_U_TURN, MOVE_KNOCK_OFF, MOVE_SUPERPOWER}
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_TANGROWTH,
    .heldItem = ITEM_ASSAULT_VEST,
    .abilityNums = 0,
    .moves = {MOVE_EARTHQUAKE, MOVE_POWER_WHIP, MOVE_KNOCK_OFF, MOVE_SLUDGE_BOMB}
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_SHELL_BELL,
    .abilityNums = 0,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_ICE_FANG, MOVE_WATERFALL}
    },
    {
    .iv = 255,
    .lvl = 82,
    .species = SPECIES_SUICUNE,
    .heldItem = ITEM_CHESTO_BERRY,
    .abilityNums = 0,
    .moves = {MOVE_CALM_MIND, MOVE_REST, MOVE_SCALD, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 83,
    .species = SPECIES_TYPHLOSION,
    .heldItem = ITEM_TRI_PASS,
    .abilityNums = 0,
    .moves = {MOVE_ERUPTION, MOVE_FIRE_BLAST, MOVE_DARK_PULSE, MOVE_EXTRASENSORY}
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Anabel[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Tucker[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Spenser[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Greta[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Noland[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Lucy[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Brandon[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 10,
    .lvl = 31,
    .species = SPECIES_BOLDORE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 20,
    .lvl = 33,
    .species = SPECIES_GIGALITH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres4[] = {
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 30,
    .lvl = 35,
    .species = SPECIES_GIGALITH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Andres5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_PROBOPASS,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_MUDSDALE,
    },
    {
    .iv = 40,
    .lvl = 37,
    .species = SPECIES_GIGALITH,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_HITMONCHAN,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 10,
    .lvl = 30,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory3[] = {
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_HITMONCHAN,
    },
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 20,
    .lvl = 32,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_HITMONCHAN,
    },
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 30,
    .lvl = 34,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cory5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_HITMONCHAN,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 40,
    .lvl = 36,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo2[] = {
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 100,
    .lvl = 70,
    .species = SPECIES_SLOWBRO_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo3[] = {
    {
    .iv = 150,
    .lvl = 72,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 150,
    .lvl = 72,
    .species = SPECIES_SLOWBRO_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo4[] = {
    {
    .iv = 200,
    .lvl = 74,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 200,
    .lvl = 74,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 200,
    .lvl = 74,
    .species = SPECIES_SLOWBRO_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Pablo5[] = {
    {
    .iv = 250,
    .lvl = 76,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 250,
    .lvl = 76,
    .species = SPECIES_LAPRAS,
    },
    {
    .iv = 250,
    .lvl = 76,
    .species = SPECIES_SLOWBRO_MEGA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji2[] = {
    {
    .iv = 10,
    .lvl = 49,
    .species = SPECIES_BEWEAR,
    },
    {
    .iv = 10,
    .lvl = 49,
    .species = SPECIES_CRABOMINABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji3[] = {
    {
    .iv = 20,
    .lvl = 51,
    .species = SPECIES_BEWEAR,
    },
    {
    .iv = 20,
    .lvl = 51,
    .species = SPECIES_CRABOMINABLE,
    },
    {
    .iv = 20,
    .lvl = 51,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji4[] = {
    {
    .iv = 30,
    .lvl = 53,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 30,
    .lvl = 53,
    .species = SPECIES_BEWEAR,
    },
    {
    .iv = 30,
    .lvl = 53,
    .species = SPECIES_CRABOMINABLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Koji5[] = {
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_CRABOMINABLE,
    },
    {
    .iv = 40,
    .lvl = 56,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin2[] = {
    {
    .iv = 110,
    .lvl = 72,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = 110,
    .lvl = 73,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 110,
    .lvl = 73,
    .species = SPECIES_GOTHITELLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin3[] = {
    {
    .iv = 150,
    .lvl = 74,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = 150,
    .lvl = 75,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 150,
    .lvl = 75,
    .species = SPECIES_GOTHITELLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin4[] = {
    {
    .iv = 200,
    .lvl = 76,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = 200,
    .lvl = 77,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 200,
    .lvl = 77,
    .species = SPECIES_GOTHITELLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Cristin5[] = {
    {
    .iv = 250,
    .lvl = 78,
    .species = SPECIES_PERRSERKER,
    },
    {
    .iv = 250,
    .lvl = 79,
    .species = SPECIES_AROMATISSE,
    },
    {
    .iv = 250,
    .lvl = 79,
    .species = SPECIES_GOTHITELLE,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando2[] = {
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 20,
    .lvl = 37,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Fernando5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 10,
    .lvl = 38,
    .species = SPECIES_GURDURR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer3[] = {
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_DUGTRIO_ALOLAN,
    },
    {
    .iv = 20,
    .lvl = 40,
    .species = SPECIES_GURDURR,
    },
    {
    .iv = 20,
    .lvl = 42,
    .species = SPECIES_AGGRON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer4[] = {
    {
    .iv = 30,
    .lvl = 43,
    .species = SPECIES_DUGTRIO_ALOLAN,
    },
    {
    .iv = 30,
    .lvl = 43,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_AGGRON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Sawyer5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_DUGTRIO_ALOLAN,
    },
    {
    .iv = 40,
    .lvl = 45,
    .species = SPECIES_CONKELDURR,
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_AGGRON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle2[] = {
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_MISDREAVUS,
    },
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_HAUNTER,
    },
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_DHELMISE,
    },
    {
    .iv = 10,
    .lvl = 41,
    .species = SPECIES_SANDYGAST,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle3[] = {
    {
    .iv = 20,
    .lvl = 43,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .iv = 20,
    .lvl = 43,
    .species = SPECIES_HAUNTER,
    },
    {
    .iv = 20,
    .lvl = 43,
    .species = SPECIES_DHELMISE,
    },
    {
    .iv = 20,
    .lvl = 43,
    .species = SPECIES_PALOSSAND,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle4[] = {
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_DHELMISE,
    },
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 30,
    .lvl = 45,
    .species = SPECIES_CURSOLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Gabrielle5[] = {
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_MISMAGIUS,
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_GENGAR,
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_DHELMISE,
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_PALOSSAND,
    },
    {
    .iv = 40,
    .lvl = 47,
    .species = SPECIES_CURSOLA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 10,
    .lvl = 34,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 20,
    .lvl = 36,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia4[] = {
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 30,
    .lvl = 38,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Thalia5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 40,
    .lvl = 40,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Mariela[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Alvaro[] = {
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_SHUPPET,
    },
    {
    .iv = 0,
    .lvl = 25,
    .species = SPECIES_ARIADOS,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Everett[] = {
    {
    .iv = 0,
    .lvl = 26,
    .species = SPECIES_WOBBUFFET,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Red[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_CHARMANDER,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_Leaf[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_BULBASAUR,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_BrendanLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_GROUDON,
    }
};

static const struct TrainerMonNoItemDefaultMoves sParty_MayLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .species = SPECIES_KYOGRE,
    }
};
